...
#endif // _UNICODE
			::WriteFile( hFile,
						 m_strTitleName.c_str(),
						 m_strTitleName.length(),
						 &dwNumBytesWritten,
						 NULL );
		} else {
...
//---------------------------------------------------------------------------
// GetHistory for link
void CLogFile::GetHistory(history_node* node, UINT nItemCount)
{
	DWORD dwFileNum = 1;
	TCHAR szCurrFileName[MAX_PATH];
	TCHAR szTempFileName[MAX_PATH];
	...
	while ( nItemCount > 0 ) {
		_stprintf(szCurrFileName, _TEXT("%s\\%s%03d.log"),
			"C:\\TOHOWORK\\Alignment\\LOG", "Align_Axis1Mon", dwFileNum);
		::GetTempFileName("C:\\TOHOWORK\\Alignment\\LOG", _T("log"), dwFileNum, szTempFileName);
		if ( !::CopyFile(szCurrFileName, szTempFileName, FALSE) ) {
			break;
		}
		hFile = ::CreateFile( szTempFileName,
		...
		pszBuffer[dwFileSize / sizeof(TCHAR)] = '\0';
		_tcsrev(pszBuffer);
		pszText = _tcstok(pszBuffer, "\n\a");
		while ( pszText != NULL && nItemCount > 0 ) {
			node->add(_tcsrev(pszText));
			nItemCount--;
			pszText = _tcstok(NULL, "\n\a");
		}
