...
// 		pCSDark->dScanData[i] = atof(tokenWave);					// _[Nf[^̎擾
// 		tokenWave = strtok(NULL, " ");
// 		pCSRef->dScanData[i] = atof(tokenWave); 					// t@Xf[^̎擾
// 		i++;
// 	}
	...
	while ( _fgetts(szLine, nMaxSize, fp) != NULL )
	{
		...
		TCHAR* tokenWave = strtok(buf, " ");
		tokenWave = strtok(NULL, " ");
		pCSDark->SetScanData(d, atof(tokenWave));					// _[Nf[^̎擾
		tokenWave = strtok(NULL, " ");
		pCSRef->SetScanData(d, atof(tokenWave)); 					// t@Xf[^̎擾
...
// 		TCHAR* tokenWave = strtok(buf, " ");
// 		tokenWave = strtok(NULL, " ");
// 		pCSDark->dScanData[i] = atof(tokenWave);					// _[NT1f[^̎擾
// 		tokenWave = strtok(NULL, " ");
// 		pCS1stRef->dScanData[i] = atof(tokenWave);					// 1stt@XT1f[^̎擾
// 		tokenWave = strtok(NULL, " ");
// 		pCS2ndRef->dScanData[i] = atof(tokenWave);					// 2stt@XT1f[^̎擾
// 		i++;
// 	}
	...
	while ( _fgetts(szLine, nMaxSize, fp) != NULL )
	{
		...
		TCHAR* tokenWave = strtok(buf, " ");
		tokenWave = strtok(NULL, " ");
		pCSDark->SetScanData(d, atof(tokenWave));					// _[NT1f[^̎擾
		tokenWave = strtok(NULL, " ");
		pCS1stRef->SetScanData(d, atof(tokenWave));					// 1stt@XT1f[^̎擾
		tokenWave = strtok(NULL, " ");
		pCS2ndRef->SetScanData(d, atof(tokenWave));					// 2stt@XT1f[^̎擾
		d += pCSDark->dWavelengthStep;
	}
// 2014.04.04 bagus wavelength step modified <--
	fclose(fp);
...
// 		TCHAR* tokenWave = strtok(buf, " ");
// 		tokenWave = strtok(NULL, " ");
// 		pCSDark->dScanData[i] = atof(tokenWave);					// _[NT2f[^̎擾
// 		tokenWave = strtok(NULL, " ");
// 		pCS2ndRef->dScanData[i] = atof(tokenWave);					// 2ndt@XT2f[^̎擾
// 		i++;
// 	}
	...
	while ( _fgetts(szLine, nMaxSize, fp) != NULL )
	{
		...
		TCHAR* tokenWave = strtok(buf, " ");
		tokenWave = strtok(NULL, " ");
		pCSDark->SetScanData(d, atof(tokenWave));					// _[NT2f[^̎擾
		tokenWave = strtok(NULL, " ");
		pCS2ndRef->SetScanData(d, atof(tokenWave));					// 2ndt@XT2f[^̎擾
		d += pCSDark->dWavelengthStep;
	}
// 2014.04.04 bagus wavelength step modified <--
	fclose(fp);
...
BOOL CSrRefFile::WriteRefFile(SCANDATA* pScanData, LPCTSTR pszMainRecipeName)
{
	...
	PSCANDATA pCSDark = &pScanData[MEASYS_DARK_T1];
	PSCANDATA pCSRef  = &pScanData[MEASYS_STDR_T1];
	fprintf(fp, "Start=%3d\n", pCSDark->iScanStart);				// wb_i1sځj
	fprintf(fp, "End=%3d\n", pCSDark->iScanEnd);					// wb_i2sځj
	fputs("nm   Dark     Reference\n", fp); 						// wb_i3sځj

// 2014.04.04 bagus wavelength step modified -->
//	int i;
	for ( i = 0; i < SCANDATA_POINT_MAX; i++ )
//		fprintf(fp, "%4d %lf %lf\n", i+1, pCSDark->dScanData[i], pCSRef->dScanData[i]); 		// ʁi4sڈȍ~j

	if(pCSDark->dWavelengthStep == 0.5){
		for ( double d = 0.5; d <= SCANDATA_POINT_MAX; d += 0.5 ) {
			fprintf(fp, "%4.1lf %lf %lf\n", d, pCSDark->GetScanData(d), pCSRef->GetScanData(d)); 	// ʁi4sڈȍ~j
		}
	}
	else{
		int i;
		...
		for ( i = 1; i <= SCANDATA_POINT_MAX; i++ ) {
			fprintf(fp, "%4d %lf %lf\n", i, pCSDark->GetScanData(i), pCSRef->GetScanData(i)); 	// ʁi4sڈȍ~j
		}
	}
// 2014.04.04 bagus wavelength step modified <--
	fclose(fp);
...
BOOL CSrRefFile::WriteGTrRefFile(SCANDATA* pScanData, LPCTSTR pszMainRecipeName)
{
	...
	strcpy(szHeader, "nm   Dark "); 								// wb_i3sځj
	int i;
...
// 2014.04.04 bagus wavelength step modified -->
// 	for ( i = 0; i < SCANDATA_POINT_MAX; i++ ) {					// ʁi4sڈȍ~j
...
// 	}
	...
	if(pCSDark->dWavelengthStep == 0.5){
		...
		for ( i = 1; i <= SCANDATA_POINT_MAX; i++ ) {					// ʁi4sڈȍ~j
...
// 	int i;
 	for ( i = 0; i < SCANDATA_POINT_MAX; i++ )
// 		fprintf(fp, "%4d %lf %lf %lf\n", i+1, pCSDark->dScanData[i], pCS1stRef->dScanData[i], pCS2ndRef->dScanData[i]); 		// ʁi4sڈȍ~j

	if(pCSDark->dWavelengthStep == 0.5){
		for ( double d = 0.5; d <= SCANDATA_POINT_MAX; d += 0.5 ) {
			fprintf(fp, "%4.1lf %lf %lf %lf\n", d, pCSDark->GetScanData(d), pCS1stRef->GetScanData(d), pCS2ndRef->GetScanData(d)); 	// ʁi4sڈȍ~j
		}
	}
	else{
		int i;
		...
		for ( i = 1; i <= SCANDATA_POINT_MAX; i++ ) {
			fprintf(fp, "%4d %lf %lf %lf\n", i, pCSDark->GetScanData(i), pCS1stRef->GetScanData(i), pCS2ndRef->GetScanData(i)); 	// ʁi4sڈȍ~j
		}
	}
// 2014.04.04 bagus wavelength step modified <--
	fclose(fp);
...
// 	int i;
 	for ( i = 0; i < SCANDATA_POINT_MAX; i++ )
// 		fprintf(fp, "%4d %lf %lf\n", i+1, pCSDark->dScanData[i],  pCS2ndRef->dScanData[i]); 		// ʁi4sڈȍ~j

	if(pCSDark->dWavelengthStep == 0.5){
		for ( double d = 0.5; d <= SCANDATA_POINT_MAX; d += 0.5 ) {
			fprintf(fp, "%4.1lf %lf %lf\n", d, pCSDark->GetScanData(d),  pCS2ndRef->GetScanData(d)); 	// ʁi4sڈȍ~j
		}
	}
	else{
		int i;
		...
		for ( i = 1; i <= SCANDATA_POINT_MAX; i++ ) {
			fprintf(fp, "%4d %lf %lf\n", i, pCSDark->GetScanData(i),  pCS2ndRef->GetScanData(i)); 		// ʁi4sڈȍ~j
		}
	}
// 2014.04.04 bagus wavelength step modified <--
	fclose(fp);
...
// 2009.09.10 K.Matsuo <--
	...
	TCHAR szPath[MAX_PATH+1];
	sprintf(szPath, "%s%s%s", g_tszData_Ref_Dir, pszMainRecipeName, DAT_EXT);
	HANDLE hFile = CreateFile(szPath, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);	// t@Cnh擾
	if ( hFile == INVALID_HANDLE_VALUE )
		return ERR_NOEXIST;    										// t@CȂꍇA؂Ƃ
	...
	GetFileTime(hFile, NULL, NULL, &ftFileTime);					// t@CŏIXV
	CloseHandle(hFile);
	FileTimeToLocalFileTime(&ftFileTime, &ftLocalFileTime); 		// t@CŏIXVi[Jj
	FileTimeToSystemTime(&ftLocalFileTime, &stLocalFileTime);		// t@CŏIXVi[JjSYSTEMTIME^
	GetLocalTime(&stLocalTime); 									// ݎi[Jj

	struct tm tmCurrTime;
	time_t timeFileTime;
	time_t timeCurrTime;
	ZeroMemory(&tmFileTime, sizeof(tmFileTime));
	tmFileTime.tm_year = stLocalFileTime.wYear - 1900;				// 1900 N̔N
	...
	//	  MessageBox(NULL, "mktime(&tmFileTime) ̎sɎs܂", "NanoSpec", MB_OK);
...
//	}
	...
	tmCurrTime.tm_year = stLocalTime.wYear - 1900;					// 1900 N̔N
	...
	//	  MessageBox(NULL, "mktime(&tmCurrTime) ̎sɎs܂", "NanoSpec", MB_OK);
