...
/* added 2014.05.22 hmenjo SC_adap timeout �΍� ---------- { ---------- */
...
/* modified 2014.11.25 hmenjo XMP �\��/��\�� ---------- } ---------- */
	...
	for(int i = 0; i < static_cast<int>(strlen(srXmp.szAdapExePath)); i++){
...
/* deleted 2014.05.22 hmenjo SC_adap timeout �΍� ---------- { ---------- */
//	BOOL bSC_Adap = FALSE;
//
//	char szLowerAdapExePath[MAX_PATH];
//	memset(szLowerAdapExePath, 0, sizeof(szLowerAdapExePath));
...
// 2014.03.08 bagus SCOUT modified <--
	...
	CDdeClient xmpDde;
	xmpDde.Init();
/* modified 2014.05.22 hmenjo SC_adap timeout �΍� ---------- { ---------- */
//	timer.Restart(10);
/* modified 2014.05.22 hmenjo SC_adap timeout �΍� ----------              */
...
/* modified 2014.05.22 hmenjo SC_adap timeout �΍� ---------- } ---------- */
	...
	xmpDde.Execute(_T("WindowState(1)"));		// XMP unvisible�ɂ���
	xmpDde.Disconnect();
	xmpDde.Uninit();
...
BOOL CXmp::DoXmp(LPTSTR pszXmpDdeText, LPCTSTR pszCmd, CDdeClient* pXmpDde)
{
	...
	// WVASE32�v�Z���̂��ߌ��ʑ҂��BADAP�́AWVASE32.EXE��FIT�l���擾����܂ł̊ԁA""��ԋp���Ă���
