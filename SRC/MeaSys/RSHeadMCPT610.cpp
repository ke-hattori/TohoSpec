...
//	コマンド
#define RESET		 		_T("RST")		// 初期化
#define AD_OFFSET	   		_T("ZRD")		// A/Dコンバータのオフセット調整を実行（約10秒間）
#define RANGE		 		_T("RNG")		// 測定レンジ設定（RNGnn, nn=-3～+7）
#define PROBE	  			_T("PRB")		// 測定プローブ設定（PRBn）
#define LIMITV 			   	_T("LMT")		// リミッタ電圧設定（LMTn, n=0:10V,1:90V）
#define MEAS_START	   		_T("MES")		// 測定開始
#define MEAS_END			_T("HLD")		// 測定終了
#define STATUSREQ	 		_T("SRQ")		// ステータス要求
#define PARAMREQ			_T("PRQ")		// パラメータ要求
#define MEAS_DATAREQ		_T("DRQ")		// 測定データ要求
#define RCF					_T("RCF")		// 抵抗率補正係数（RCFXXXXX, XXXXX=0.001～9999（最大5文字））
#define THICKDATA			_T("THK")		// 試料厚みデータ（THKXXXXX##, XXXXX=0.001～9999（最大5文字）, ##=MM,UM,NM,AM）
...
BOOL CRsHeadMCPT610::InitInstance()
{
	...
	// ロレスタ取説のサンプルコードが初期化コマンドを2回送信しているので、マネをした
	for ( int i = 0; i < 2; i++ ) {
		if ( !Reset() )
			return FALSE;
	}

	// 現在の設定パラメータ情報を取得する。また、通信できるかのチェックも兼ねて、応答を返すコマンドを送信している
...
BOOL CRsHeadMCPT610::SendCommand(LPCTSTR pszCmd)
{
	...
	for ( int iTry = 0; iTry < MAXTIMES; iTry++ ) {
		...
		bySend = (BYTE)_TCHAR('\n');
		if ( !::WriteFile(m_hComm, &bySend, 1, &dwNumberOfBytesWritten, NULL) ) // 終端文字LFの設定
			continue;		// Error -> Retry
...
BOOL CRsHeadMCPT610::RecvData()
{
	...
	for ( int i = 0; i < RS232C_RECV_CHAR; i++ ) {
		...
		if ( m_pszRecvBuff[i] == _TCHAR('\n') ) {
			m_pszRecvBuff[i] = NULL;								// 終端文字LFをNULL文字に置換
			break;
		}
	}
	if ( i == RS232C_RECV_CHAR ) {									// 終端文字LFが見つからなかった
		return FALSE;
	}
