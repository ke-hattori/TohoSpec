...
/* added 2009.10.20 hmenjo CTA ǉ(P) ---------- { ---------- */
...
// 2013.02.01 bagus CompleteEASEwbhǉ MeaSys.hxxֈړ -->
...
CSharedMemory<RESISTRESULT> smResistResultDataBase; /* RS 茋 DB LGA	*/
/* added 2009.10.27 hmenjo CTA 茋 DB ---------- { ---------- */
CSharedMemory<CTARESULT> lg_smCtaResultDataBase;	/* CTA 茋 DB LGA	*/
/* added 2009.10.27 hmenjo CTA 茋 DB ---------- } ---------- */
// 2013.02.01 bagus CompleteEASEwbhǉ -->
...
TCHAR g_tszProcDir[_MAX_PATH] = _T(""); 	/* ďovZX̃fBNg('\'t)*/          /* C:\NanoSpec\BIN\ or C:\NanoSpec\BIN_DEB\ */
TCHAR g_tszBaseDir[_MAX_PATH] = _T(""); 	/* fBNg('\'t)*/                      /* C:\NanoSpec\ */
	...
	)
{
	if (0 == _tcscmp(ptszProcDir, _T(""))) {
		TCHAR l_tszProcessFName[_MAX_PATH]; /* ďovZX̃tpX*/
...
{
// 2009.10.02 K.Matsuo XgXSRŖ肪̂ŁAȂ -->
...
/* deleted 2009.10.28 hmenjo CTA bgCTvݒ֐ ---------- { ---------- */
///* added 2009.10.27 hmenjo CTA bgCTvݒ֐ ---------- { ---------- */
...
// 2013.02.01 bagus CompleteEASEwbhǉ <--
	case HEAD_TYPE_4PP:
		...
		break;
/* added 2009.10.26 hmenjo CTA MEAS_Measure()ďo ---------- { ---------- */
	case HEAD_TYPE_CTA:
		if (0 == lgs_pclsCCtaCtrl) {
			bRet = FALSE;
		} else {
...
// 2009.11.20 K.Matsuo GTr ݂̃t@Xf[^cĂ̂ŏ -->
...
// 2009.10.26 K.Matsuo TR -->
	if ( MEASYS_GTR_1 <= iScanCnt && iScanCnt <= MEASYS_GTR_LAST ) {
		// SR GTriKg[ߗj̏ꍇAt@X|Cgԍ̈ʒuɁAf[^i[
...
/* added 2019.02.01 hmenjo Oǉ Ph1 ---------- { ---------- */
...
/* added 2019.02.01 hmenjo Oǉ Ph1 ---------- } ---------- */
			...
			token = _tcstok(pszAdapResult, _T("=,"));			// ŏ̃g[N擾܂
			while ( token != NULL )
			{
				// 2009.08.19 K.Matsuo -->
...
// 2009.11.12 K.Matsuo ADAP̕񍐍ڂ̐ΉB2ڂN1[XXX],K1[XXX]̂ƂAʍڂ̒l -->
...
// 2009.11.12 K.Matsuo ADAP̕񍐍ڂ̐ΉB2ڂN1[XXX],K1[XXX]̂ƂAʍڂ̒l <--
				...
				if ( wHeadType == HEAD_TYPE_SR ) {
					...
					for ( i = 0; i < RECALIB_MAX; i++ ) {
						if ( g_prepareData.pMainRcpInfo->MainRcpParam._SR.RecalibItem[i] == 0 ) 				// RecalibItemindex 0́Askip
							continue;
						if ( iCol == (g_prepareData.pMainRcpInfo->MainRcpParam._SR.RecalibItem[i] - 1) ) {	// RecalibItemindex́A1n܂邽߁A1Z
...
// 2009.11.12 K.Matsuo ADAP̕񍐍ڂ̐ΉB2ڂN1[XXX],K1[XXX]̂ƂAʍڂ̒l <--
			...
			for ( i = 0; i < RECALIB_MAX; i++ ) {
				if ( g_prepareData.pMainRcpInfo->MainRcpParam._SE.RecalibItem[i] == 0 ) 			// RecalibItemindex 0́Askip
					continue;
				if ( iCol == (g_prepareData.pMainRcpInfo->MainRcpParam._SE.RecalibItem[i] - 1) ) {	// RecalibItemindex́A1n܂邽߁A1Z
...
// 2010.02.01 K.Matsuo eXg[hA0ł̂ŁAVXeG[sv֎戵ύX -->
//		// Lu[Ṽf[^wiVXeG[j
//		return FALSE;
		return TRUE;
// 2010.02.01 K.Matsuo eXg[hA0ł̂ŁAVXeG[sv֎戵ύX <--
...
// 2009.11.16 K.Matsuo Lu[VvEsv <--
	...
	if ( !RecipeFile_LoadRecipe(&recalibProgInfo, szRecalib, RECIPE_FILE_RECALIBRATION_PROGRAM) )
		return FALSE;
/* added 2010.05.11 hmenjo LuPʃoOC ---------- { ---------- */
...
/* added 2010.05.11 hmenjo LuPʃoOC ---------- } ---------- */
	...
	for ( i = 0; i < RECALIB_MAX_ENTRY; i++ )
		l_RecalibEntry[i+1] = recalibProgInfo.RecalibEntry[i];					// index0́Aactual=0.0,measured=0.0Ƃ
/* added 2015.12.06 hmenjo Jut֑Ή ---------- { ---------- */
	if (2 <= recalibProgInfo.iEntryNum) {
		/* v2.10 ͋tփf[^݂͑܂̂ŏ܂D	*/
		if (l_RecalibEntry[1].dActual > l_RecalibEntry[2].dActual) {
/* modified 2015.12.10 hmenjo Jut֑Ή ---------- { ---------- */
//			/* l̃ItZbgPQ̕ωȌꍇ
//				l̃ItZbgOItZbgPƓlɂ܂	*/
//			l_RecalibEntry[0].dActual = l_RecalibEntry[1].dActual;
/* modified 2015.12.10 hmenjo Jut֑Ή ----------              */
			/* l̃ItZbgPQ̕ωȌꍇ
				l̃ItZbgOItZbgPQ̌Xŋ߂܂D	*/
	...
	if ( l_RecalibEntry[i].dMeasured < dData ) {
		if ( recalibProgInfo.iEntryNum == -1 ) {
			...
			for ( i = 1; i < iMaxEntryNum; i++ ) {
				if ( i > 1 && l_RecalibEntry[i].dMeasured == 0.0 ) {	// f[^sȂ̂ŁA0.0Xgbp[Ƃ݂ȂĂ
					i -= 2;
					break;
				}
				if ( l_RecalibEntry[i].dMeasured >= dData ) {
			...
			if ( l_RecalibEntry[i + 1].dMeasured == 0.0 )				// f[^sȂ̂ŁA0.0Xgbp[Ƃ݂ȂĂ
				i -= 1;
		}
		else {
			// v2.10ȍ~̃WbN
...
// 2009.09.15 K.Matsuo Lu[Ṽ}CiXl͑Ή -->
// --------------------------------------------------------------------
...
// 2009.12.07 K.Matsuo XgeW[ԍ1́AXgΏۊO -->
...
//	 "(STR)"܂ރXgeW[́AXgXp̃XgeW[B
//	iADAPXgeW[쐬҂Ƃ̃l[~O[ŉ^pĂBj
//	 XgXpXgeW[́A1wiThick1jŒ`Ă͂łA
//	 ۂwłĂA1wɂ܂Ƃ߂Ē`ꂽAʂȃXgeW[쐬ĂB
...
/* added 2009.10.23 hmenjo CTA API `ǉ ---------- { ---------- */
/*
 *	CTA 胂[hݒ
 *		MEAS_CTA_MODE_1 [hPFPʁEIy[^f[^mFȂ
 *		MEAS_CTA_MODE_2 [hQFPʁEIy[^f[^mF
 *		MEAS_CTA_MODE_3 [hRFJZbgPʁEIy[^f[^mF
 */
...
BOOL MEASYSAPI MEAS_CtaSetThick(
		DWORD dwThick			/* ݏ[0.1mm]C͈(0.0`10.0mm)	*/
	)
{
	if (0 == lgs_pclsCCtaCtrl) {
		return FALSE;
...
BOOL MEASYSAPI MEAS_CtaSetCstSample(
		...
		const MAIN_RCP_INFO* pMainRcpInfo	/* CVs̃AhX	*/
	)
{
	if (0 == lgs_pclsCCtaCtrl) {
		return FALSE;
