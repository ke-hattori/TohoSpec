...
// 2009.05.18 K.Matsuo oOΉ Δ˗100%ĂA100%ɂĂΉ -->
			// ̎_́A܂Δ˗ł̂ŁÂߍ폜܂BBB
//			if ( ResultData[i] > 1.0 )
//				ResultData[i] = 1.0;
// 2009.05.18 K.Matsuo oOΉ Δ˗100%ĂA100%ɂĂΉ <--
...
//'====================================================
//'
//'   Fx荀ڊevZPart1iXYZ,XnYnZnj
//'   COD_SdataModif[^FxvZ
//'   Lu[V@\̂߂ɂQɕ
//'   Part1ł́uXYZv߁ALuAPart2ɂĕ␳XYZɂČvZ
//'
//'====================================================
//*/
//
...
//	  cieStarV As Double
//
//	  cieTempXn As Double	 'vZtempiLu[V̂tempO[oKvj
//	  cieTempYn As Double	 'vZtempiLu[V̂tempO[oKvj
//	  cieTempZn As Double	 'vZtempiLu[V̂tempO[oKvj
//End Type
...
//'====================================================
//'
//'   Fx荀ڊevZPart2ixyz,Lab,a*b*,u'v',u*v*j
//'   COD_SdataModif[^FxvZ
//'   Lu[V@\̂߂ɂQɕ
//'  Part1ł́uXYZv߁ALuAPart2ɂĕ␳XYZɂČvZ
//'
...
//	  }
//
//	  //'[u'n,v'n (Complete refrection)]\Ȃ    // SgUʂŌvZu',v'l
...
BOOL CSrAnalysis::DoXmp(LPTSTR pszXmpDdeText, ANALYSISDATA* pAnalysisData, const MAIN_RCP_INFO* pMainRcpInfo, const MEAS_PROG_INFO* pMeasProgInfo, CDdeClient* pXmpDde)
{
	...
	// FITDATAR}hM
// 2009.09.14 K.Matsuo NanoSpec6500,9300R}h ݊Ή -->
//		6500 : strategy
//		9300 : StratNum
...
/* added 2016.02.24 hmenjo ob` ---------- } ---------- */
	...
	if ( (fp = fopen(pszFullPath, _T("w"))) == NULL )  {
		_stprintf(szBuff, _T("%s%s"), pszFullPath, _T("̍쐬Ɏs܂"));
...
// 2014.04.03 bagus wavelength step modified <--
	...
	if ( !file.Open(pszFullPath, CFile::modeRead | CFile::typeText | CFile::shareDenyNone) )  {
		_stprintf(szBuff, _T("%s%s"), pszFullPath, _T("̓Ǎ݂Ɏs܂"));
//		::MessageBox(NULL, szBuff, _T("NanoSpec"), MB_OK | MB_ICONERROR);
		return FALSE;
	}
...
{
	BOOL l_bRc = FALSE;
	static TCHAR ls_tszAft2AdapPathSrc[MAX_PATH] = _T("");	/* OIt@C̃tpX	*/
	...
	if (-1 != ::GetFileAttributes(ls_tszAft2AdapPathSrc)) {
		...
		if (FALSE != l_bDef) {
			l_bDef = FALSE;
			/* ptszFullPath ̃fBNgg܂D	*/
			_tcscpy(l_tszAft2AdapDir, ptszFullPath);
			l_ptszNull = _tcsrchr(l_tszAft2AdapDir, _T('\\'));
			/* ptszFullPath ̃fBNg݂͑邩HH	*/
			if (0 == l_ptszNull) {
				/* pX'\'	*/
				l_bDef = TRUE;
			} else {
	...
	l_OpenFileName.Flags =
						OFN_FILEMUSTEXIST |		/* ݂pX̂ݗL	*/
						OFN_PATHMUSTEXIST |		/* ݂t@ĈݗL	*/
