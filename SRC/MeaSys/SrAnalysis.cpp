...
// 2009.05.18 K.Matsuo バグ対応 絶対反射率で100%超えていたら、100%にしておく対応 -->
			// この時点は、まだ相対反射率でしたので、誤りのため削除しました。。。
//			if ( ResultData[i] > 1.0 )
//				ResultData[i] = 1.0;
// 2009.05.18 K.Matsuo バグ対応 絶対反射率で100%超えていたら、100%にしておく対応 <--
		}
...
//'====================================================
//'
//'   色度測定項目各種計算Part1（XYZ,XnYnZn）
//'   COD_SdataModiデータから色度を計算
//'   リキャリブレーション機能のために２つに分割
//'   Part1では「XYZ」求め、リキャリブ後、Part2にて補正後のXYZにて計算
//'
//'====================================================
//*/
//
...
//		::MessageBox(NULL, _T("CIE イルミナント計算失敗 ReadIllumiFile"), _T("NanoSpec"), MB_OK | MB_ICONERROR);
//		return;
//	}
//	if ( !ReadCIECMFFile() ) {
//		::MessageBox(NULL, _T("CIE イルミナント計算失敗 ReadCIECMFFile"), _T("NanoSpec"), MB_OK | MB_ICONERROR);
//		return;
//	}
//
...
//	  cieStarV As Double
//
//	  cieTempXn As Double	 '計算temp（リキャリブレーションのためtempもグローバル化が必要）
//	  cieTempYn As Double	 '計算temp（リキャリブレーションのためtempもグローバル化が必要）
//	  cieTempZn As Double	 '計算temp（リキャリブレーションのためtempもグローバル化が必要）
//End Type
...
//'====================================================
//'
//'   色度測定項目各種計算Part2（xyz,Lab,a*b*,u'v',u*v*）
//'   COD_SdataModiデータから色度を計算
//'   リキャリブレーション機能のために２つに分割
//'  Part1では「XYZ」求め、リキャリブ後、Part2にて補正後のXYZにて計算
//'
...
//		  CIE_Result.cieSmallz = CIE_Result.cieLargeZ / (CIE_Result.cieLargeX + CIE_Result.cieLargeY + CIE_Result.cieLargeZ); // Smallz = 1 - Smallx - Smally と同じ
//	  }
//
//	  //'[L,a,b]
...
//	  }
//
//	  //'[u'n,v'n (Complete refrection)]表示なし    // 完全拡散面で計算したu',v'値
...
//	  End If
//
//	  '[u'n,v'n (Complete refrection)]表示なし
...
/* added 2009.07.07 hmenjo dll 相対パス対応 MeaSys.dll ---------- } ---------- */
	...
	if ( (fp = fopen(szFilename, _T("r"))) == NULL ) {
		TCHAR szBuff[256];
		_stprintf(szBuff, _T("%s%s"), szFilename, _T("がありません"));
...
BOOL CSrAnalysis::DoXmp(LPTSTR pszXmpDdeText, ANALYSISDATA* pAnalysisData, const MAIN_RCP_INFO* pMainRcpInfo, const MEAS_PROG_INFO* pMeasProgInfo, CDdeClient* pXmpDde)
{
	...
	// FITDATAコマンド送信
// 2009.09.14 K.Matsuo NanoSpec6500,9300コマンド 互換対応 -->
//		6500 : strategy
//		9300 : StratNum
...
/* added 2016.02.24 hmenjo バッチ処理 ---------- { ---------- */
	if (2 == g_lBatchFlag) {
		...
		if (FALSE != this->CopyAft2AdapFileSpecified(dWavelengthStep, dXmpAngle, pszFullPath)) {
			l_bRslt = TRUE;
/* added 2016.07.08 hmenjo バッチ処理2 ---------- { ---------- */
			if ((pMeasProgInfo->ScanParams.hdr.wScanType == MEAS_PROG_TYPE_SR_REFLECTANCE) ||
				...
				/* 反射率の場合は内部変数(pAnalysisData)にセットします．	*/
				...
				if (0 == (fp = fopen(pszFullPath, _T("r")))) {
					...
					if ((ll_dScanStart < 0) || (ll_dScanEnd < 0)) {
						...
						*ll_piScanStart = ll_dScanStart;
						*ll_piScanEnd = ll_dScanEnd;
					}
				}
			}
/* added 2016.07.08 hmenjo バッチ処理2 ---------- } ---------- */
...
/* added 2016.02.24 hmenjo バッチ処理 ---------- } ---------- */
	...
	if ( (fp = fopen(pszFullPath, _T("w"))) == NULL )  {
		_stprintf(szBuff, _T("%s%s"), pszFullPath, _T("の作成に失敗しました"));
...
// 2014.04.03 bagus wavelength step modified <--
	...
	if ( !file.Open(pszFullPath, CFile::modeRead | CFile::typeText | CFile::shareDenyNone) )  {
		_stprintf(szBuff, _T("%s%s"), pszFullPath, _T("の読込みに失敗しました"));
//		::MessageBox(NULL, szBuff, _T("NanoSpec"), MB_OK | MB_ICONERROR);
		return FALSE;
	}
...
//
//'CIE 1931 2-deg XYZ Color Matching Functions定義値配列
...
{
	BOOL l_bRc = FALSE;
	static TCHAR ls_tszAft2AdapPathSrc[MAX_PATH] = _T("");	/* 前回選択したファイルのフルパス	*/
	...
	if (-1 != ::GetFileAttributes(ls_tszAft2AdapPathSrc)) {
		...
		if (FALSE != l_bDef) {
			l_bDef = FALSE;
			/* ptszFullPath のディレクトリを使います．	*/
			_tcscpy(l_tszAft2AdapDir, ptszFullPath);
			l_ptszNull = _tcsrchr(l_tszAft2AdapDir, _T('\\'));
			/* ptszFullPath のディレクトリは存在するか？？	*/
			if (0 == l_ptszNull) {
				/* パスに'\'が無い	*/
				l_bDef = TRUE;
			} else {
	...
	l_OpenFileName.lpfnHook = OFNHookProcOldStyle;	/* フックプロシージャ	*/
	l_OpenFileName.Flags =
						OFN_FILEMUSTEXIST |		/* 存在するパスのみ有効	*/
						OFN_PATHMUSTEXIST |		/* 存在するファイルのみ有効	*/
						OFN_NONETWORKBUTTON |	/* ネットワークボタン非表示	*/
						OFN_ENABLEHOOK |		/* フックプロシージャを有効	*/
						OFN_EXPLORER |			/* 新しいエクスプローラスタイル	*/
						0;
	/* カレントディレクトリを保存	*/
...
//		::MessageBox(l_hwndMainWnd, l_tszAft2AdapPathSrc, l_tszAft2AdapPathSrc, MB_OK);
		...
		if (0 == _tcsicmp(ptszFullPath, l_tszAft2AdapPathSrc)) {
			/* 同名の場合はテンポラリにコピーします(_mktemp は使いません)．	*/
