...
BOOL CSrHeadParallel::InitInstance()
{
	...
	if ( !WaitForStatusChange(DP_COMPLETE, DUALPORTRAM_TIMEOUT) )
		return FALSE;
	if ( !RecvData(szRecvBuff) )									// "a,00,A"を受信できれば正常
		return FALSE;
	...
	if ( !WaitForStatusChange(DP_COMPLETE, DUALPORTRAM_TIMEOUT) )
		return FALSE;
	if ( !RecvData(szRecvBuff) )									// "b,00"を受信できれば正常
		return FALSE;
	if ( !CheckCmdAndStatusCode(szSendBuff, szRecvBuff) )
		return FALSE;
...
//---------------------------------------------------------------------------
// CcdScan
BOOL CSrHeadParallel::CcdScan(int ccdScanData[], int iPixels, int iExposure, int iScans/*=1*/, BOOL bProhibitNotify/*=FALSE*/)	  // ccdScanData[iPixels]確保されていること
{
...
{
	// CCD 1ドットは、3バイト（24ビット）単位でデータ受信する。
	// スキャン回数1回の時のMAX値は、65535（16ビットMAX）だが
	// スキャン回数2,4,8回の時は、それぞれ回数分乗算した値がMAXとなるので
	// 24ビット分エリアが用意されている
	...
	for ( int i = 0; i < 3; i++ ) {
		while ( (iFifoRet = FifoRead()) == IOBASE_ERR_INVALID_ACCESS ) {   // データが取得できないタイミングがある
			if ( timer.IsTimeout() )
				return IOBASE_ERR_INVALID_ACCESS;
		}
		iCcdData += iFifoRet << (8 * i);						// iFifoRet を (8bit * i)分シフトして加算
		timer.Restart(iTimeoutSec);
	}
	return iCcdData;
}
...
void CSrHeadParallel::SendCommand(LPCTSTR pszCmd)
{
	...
	DualPortRam_Write(DP_RAM_SEND_BUFFER + i, _TCHAR('\r'));	// 終端文字CRの設定
	SetStatus(DP_SEND);
}
...
BOOL CSrHeadParallel::RecvData(LPTSTR pszData)
{
	...
	if ( i == DP_RAM_MAX_RCV_CHAR ) {							// 終端文字CRが見つからなかった
		SetStatus(DP_IDLE);
		return FALSE;
	}
