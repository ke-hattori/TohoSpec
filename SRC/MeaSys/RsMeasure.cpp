...
extern CSharedMemory<RESISTRESULT> smResistResultDataBase;	/* RS 測定結果 DB 共有エリア	*/


LPCTSTR CRsMeasure::m_pszRange[] = {"-3", "-2", "-1", "+0", "+1", "+2", "+3", "+4", "+5", "+6", "+7",}; // レシピ画面のコンボボックスと同じ順序であること
LPCTSTR CRsMeasure::m_pszThickUnit[] = {"MM", "UM", "NM", "AM",};	// レシピ画面のコンボボックスと同じ順序であること
...
BOOL CRsMeasure::Measure(const MEAS_PROG_INFO* pMeasProgInfo)
{
	...
	// プローブヘッドを基板に押し付けるため下降動作させる（＝測定位置へ移動させる）
	...
	const int MAXTIMES = 20;		/* レンジが一定方向に昇降しない場合（+0,+1,+0,+1...など）の無限ループ回避 */
	for ( int iTry = 0; iTry < MAXTIMES; iTry++ ) {
		...
		// 測定結果データを仮入れ
		// （後続のオーバーレンジ・オーバーロード・アンダーレンジチェックでリトライ上書きされる場合あり）
...
BOOL CRsMeasure::MoveProbeHeadLowerPos(BOOL bLowerPos/*=TRUE*/)
{
	...
	// インターロック監視（プローブヘッド位置が下降端になっているか）
