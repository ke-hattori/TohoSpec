...
extern CSharedMemory<ADAPRESULTDATABASE> smAdapResultDataBase;	/* ADAP 測定結果 DB 共有エリア	*/
extern CSharedMemory<COMPEASERESULT_HELPER> smCompEASEResultHelperDataBase;	/* CompleteEASE 測定結果 Helper DB 共有エリア	*/
...
BOOL CCompEASEMeasure::SetToAdapResultDataBase(DWORD dwPointNo, LPCTSTR pszMeasResult, const MAIN_RCP_INFO* pMainRcpInfo)
{
	...
	for ( int iResult = 0; iResult < strResults.GetSize(); iResult++ ) {
		...
		for  ( int i = 0; i < RECALIB_MAX; i++ ) {
			if ( pMainRcpInfo->MainRcpParam._COMPEASE.RecalibItem[i] == 0 ) 		// RecalibItemのindex 0は、skip
				continue;
			if ( iIndex == (pMainRcpInfo->MainRcpParam._COMPEASE.RecalibItem[i] - 1) ) {	// RecalibItemのindexは、1から始まるため、1減算
