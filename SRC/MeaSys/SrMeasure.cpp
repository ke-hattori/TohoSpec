...
BOOL CSrMeasure::Measure(SCANDATA scanData[], int ccdData[], int iMeaSysStatus, const MEAS_PROG_INFO* pMeasProgInfo)
{
	...
	// UvMeasure or VisMeasure or Both!?	// @@@ Vb^[J́A{ΉŁAchiefSɂȂ肻
	// g̊JnEIݒ
...
//---------------------------------------------------------------------------
// UvFinally										/// @@@ Vb^[J́A{ΉŁAchiefSɂȂ肻
...
BOOL CSrMeasure::OpenUvShutter()
{
	...
	const double dOpenDelay  = 0.8; //'[SEC]                                    // 슮MāA҂Ȃ
...
BOOL CSrMeasure::CloseUvShutter()
{
	...
	const double dCloseDelay = 0.5; //'[SEC]                                    // 슮MāA҂Ȃ
...
BOOL CSrMeasure::OpenTransShutter()
{
	...
	const double dOpenDelay = 0.5; //'[SEC]                                     // 슮MāA҂Ȃ
...
void CSrMeasure::GetScanData_Step_1nm(double* DstArray, int MinTRWave, int MaxTRWave, const int iCcdDataArray[])
{
	...
	// gŠʎZo֐
	// gۂ̔gƂɑΉʂZoi1nmj
	// Egɑ΂ۂ̃fBeN^ʒúAgiPix = m_ccdPosFitting.GetFitValue(double(p))ŋ߂
	// Eۂ̔ǧʂ́A2_`dZo@ŋ߂
	//	 i3_䗦ŋ߉d2_ZoA2_̐`䗦ŉdj
	int iPix;
	double dPix;
	double ddPix;
	double ddm;
	...
	for ( p = MinTRWave; p < MaxTRWave+1; p++ ) {
		...
		iPix = (int)dPix;
// dόvZ̊֌WŁACfbNXQƂsȈʒuɂȂ̂ŁA[̂P܂łpȂ悤ɏC
//		  if ( iPix <= 0 )
...
/* added 2009.07.07 hmenjo dll ΃pXΉ MeaSys.dll ---------- } ---------- */
	...
	while ( _fgetts(szLine, nMaxSize, fp) != NULL && cb->datanum < CALIBFILEMAXLINE )
	{
		...
		while ( *tokenWave == _TCHAR(' ') ) 							// Xy[Xǂݔ΂
			tokenWave = _tcsinc(tokenWave);

		TCHAR* psz = _tcsstr(szLine, _T("nm")); 						// gAnm̒PʕtŏĂꍇANULLɂ
		if ( psz != NULL )
			*psz = NULL;
...
void CSrMeasure::MakeScanDataLog(const int iCcdDataArray[], const double* DstArray, WORD wScanType, int iMeaSysStatus)
{
	...
	_ftprintf(fp, _T("MTYPE: %s\n"), MEAS_PROG_TYPE_SR_ITEM[wScanType]);				// wb_iށj
