...
CStressAnalysis::CStressAnalysis()
{
	...
	for ( int i = 0; i < STRESS_LINES_MAX; i++ ) {
		...
		m_pFilmThicknessFitting[SINGLE][i] = new CCurveFitting(CCurveFitting::COEF7);	// 6次式
	}
...
BOOL CStressAnalysis::CalcStress(int iLine)
{
	...
	if ( m_substrateR[FIRST][iLine].m_DeflectionAtZeroData.GetCount() == 0 ) {
		...
		// 特別処理。ライン測定を継続させるためにTRUE
		return TRUE;
	}

	/* First（素ガラス）とSingle（膜付サンプル）の曲率半径を求めます */
	double dSectionStartX;
	double dSectionEndX;

//	CalcDeflectionStartAndEndAtZero(FIRST, iLine);					// FIRST測定時のReferenceData()で実施済み
...
// 2009.12.16 K.Matsuo 2で割り切れない数字があるので、小数点以下切捨て -->
			d = (int)((dStressStartmm + dStressEndmm) / 2);			// PIN間の中央
// 2009.12.16 K.Matsuo 2で割り切れない数字があるので、小数点以下切捨て <--
			dStress = StoneyFormula(iLine, d);
// 2009.11.16 K.Matsuo リキャリブレーション対応 -->
...
//	TRACE(_T("CStressAnalysis::StoneyFormula()\n"));
	...
	if ( 0.0 ==	((m_substrateR[SINGLE][iLine].GetDthetaDx(dPosmm))	/* dθ/dx(=1/R) */ -
				 (m_substrateR[FIRST][iLine].GetDthetaDx(dPosmm)))) {
		// Zero Devide
		dStress = 0.0;
	}
		...
		dStress = (
			  ...
			  (
			   ...
			   (1 / ((m_substrateR[SINGLE][iLine].GetDthetaDx(dPosmm))	/* dθ/dx(=1/R) */ -
					 (m_substrateR[FIRST][iLine].GetDthetaDx(dPosmm))))
			  );
	}
