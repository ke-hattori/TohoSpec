...
void CButtonST::PreSubclassWindow()
{
	...
	if (nBS & BS_AUTORADIOBUTTON)	m_bIsCheckBox = TRUE;
...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	//�`�F�b�N�{�b�N�X�݂̂�\������ꍇ�ɂ͂����ŏ����B
	if(!m_csBitmaps[0].hBitmap && !m_csIcons[0].hIcon && sTitle.IsEmpty() && m_bIsCheckBox){
		int nBS = GetButtonStyle();
		RECT rcCheck = lpDIS->rcItem;
		rcCheck.top = rcCheck.top + 1;
		rcCheck.bottom = rcCheck.bottom - 1;
		if(nBS & BS_LEFTTEXT){
			rcCheck.right = rcCheck.right - 2;
			rcCheck.left = rcCheck.right - 20;
		}else{
			rcCheck.left = 2;
			rcCheck.right = 20;
		}

		pDC->DrawFrameControl(&rcCheck,DFC_BUTTON,DFCS_BUTTONCHECK | (m_nCheck ? DFCS_CHECKED : 0));
	}
	//2009.09.10 bagus stress --}--
...
void CButtonST::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	HWND hwnd;
	int i;
	for(i = 0; pszWindowTable[i] != NULL; i++){
		hwnd = ::FindWindow(NULL, pszWindowTable[i]);
		if(hwnd != NULL){
...
#else
	HWND hwnd;
	hwnd = this->GetParent()->m_hWnd;
	if(hwnd != NULL){
		::SendMessage(hwnd, WM_LBUTTONDOWN, 0, 0);
	}
#endif
//	TRACE("CButtonST::OnLButtonDown\n");
