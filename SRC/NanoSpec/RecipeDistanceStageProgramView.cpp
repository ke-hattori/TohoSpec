...
//	iRow = CellID.row;
	...
	for(i = 0 ; i < m_iNumScans + 1; i++){
		if (( m_dPointBuff[2][nIndex] != FALSE ) && ( m_dPointBuff[2][nIndex+1] != FALSE )) {
			m_ScanPoint[nIndex].lX = static_cast<long>(static_cast<float>(m_dPointBuff[0][nIndex] * MICROMETRE ));
			m_ScanPoint[nIndex].lY = static_cast<long>(static_cast<float>(m_dPointBuff[1][nIndex] * MICROMETRE ));
			m_ScanPoint[nIndex+1].lX = static_cast<long>(static_cast<float>(m_dPointBuff[0][nIndex+1] * MICROMETRE ));
			m_ScanPoint[nIndex+1].lY = static_cast<long>(static_cast<float>(m_dPointBuff[1][nIndex+1] * MICROMETRE ));
		}
...
// 2009.10.24 bagus 2点間 修正 --}--
	...
	m_StageProgInfoHdr.SampleInfo.Origin.lX = static_cast<long>(static_cast<float>(atof(strBuffer) * MICROMETRE));

	///// Sample - Origin Y /////
	GetDlgItemText(IDC_SAMPLE_ORIGIN_Y, strBuffer);
	m_StageProgInfoHdr.SampleInfo.Origin.lY = static_cast<long>(static_cast<float>(atof(strBuffer) * MICROMETRE));
...
//			m_ScanPoint[nIndex].lX = atof(szGridText) * MICROMETRE;
			lTmp = static_cast<long>(static_cast<float>(atof(szGridText) * MICROMETRE));
...
//	double dMinX, dMaxX, dMinY, dMaxY;
	...
	case 1: // 中心
		coordLeftTop.lX = static_cast<long>(static_cast<float>(m_StageProgInfoHdr.SampleInfo.Size.dx / 2 * -1));
		coordLeftTop.lY = static_cast<long>(static_cast<float>(m_StageProgInfoHdr.SampleInfo.Size.dy / 2));
		coordRightBottom.lX = static_cast<long>(static_cast<float>(m_StageProgInfoHdr.SampleInfo.Size.dx / 2));
		coordRightBottom.lY = static_cast<long>(static_cast<float>(m_StageProgInfoHdr.SampleInfo.Size.dy / 2 * -1));
		break;
	case 2: // 左上
		coordLeftTop.lX = 0;
		coordLeftTop.lY = 0;
		coordRightBottom.lX = static_cast<long>(static_cast<float>(m_StageProgInfoHdr.SampleInfo.Size.dx));
		coordRightBottom.lY = static_cast<long>(static_cast<float>(m_StageProgInfoHdr.SampleInfo.Size.dy * -1));
		break;
	case 3: // 左下
		coordLeftTop.lX = 0;
		coordLeftTop.lY = static_cast<long>(static_cast<float>(m_StageProgInfoHdr.SampleInfo.Size.dy));
		coordRightBottom.lX = static_cast<long>(static_cast<float>(m_StageProgInfoHdr.SampleInfo.Size.dx));
		coordRightBottom.lY = 0;
		break;
	case 4: // 右下
		coordLeftTop.lX = static_cast<long>(static_cast<float>(m_StageProgInfoHdr.SampleInfo.Size.dx * -1));
		coordLeftTop.lY = static_cast<long>(static_cast<float>(m_StageProgInfoHdr.SampleInfo.Size.dy));
...
// 2009.12.09 bagus --}--
	...
	if((dStageMaxSizeX / dStageMaxSizeY) > ((StageRectRight - StageRectLeft) / (StageRectBottom - StageRectTop))) {
		...
		lDrawStageSizeY = static_cast<long>(static_cast<float>(((StageRectRight - StageRectLeft) * dStageMaxSizeY / dStageMaxSizeX)));
		...
		lDrawStageSizeX = static_cast<long>(static_cast<float>(((StageRectBottom - StageRectTop) * dStageMaxSizeX / dStageMaxSizeY)));
...
// 2013.01.09 bagus <--
		...
		if ((m_dDrawStartStagePosX <= dleft) && (dright <= m_dDrawEndStagePosX) && (m_dDrawStartStagePosY <= dtop) && (dbottom <= m_dDrawEndStagePosY)){
			...
			Sampleplacement.rcNormalPosition.bottom = static_cast<long>(static_cast<float>(Stageplacement.rcNormalPosition.top  + dbottom));
			//四角Sample描画
...
BOOL CRecipeDistanceStageProgramView::CheckValidPointX(long lPointX)
{
	...
	case 1:		// Center
	default:
		lMax = static_cast<long>(static_cast<float>(m_StageProgInfoHdr.SampleInfo.Size.dx / 2));
		lMin = -lMax;
		break;
	case 2:		// Left & Top
	case 3:		// Left & Bottom
		lMax = static_cast<long>(static_cast<float>(m_StageProgInfoHdr.SampleInfo.Size.dx));
		lMin = 0;
		break;
	case 4:		// Right & Bottom
	case 5:		// Right & Top
		lMax = 0;
		lMin = static_cast<long>(static_cast<float>(-m_StageProgInfoHdr.SampleInfo.Size.dx));
		break;
	}
...
BOOL CRecipeDistanceStageProgramView::CheckValidPointY(long lPointY)
{
	...
	case 1:		// Center
	default:
		lMax = static_cast<long>(static_cast<float>(m_StageProgInfoHdr.SampleInfo.Size.dy / 2));
		lMin = -lMax;
		break;
	case 2:		// Left & Top
	case 5:		// Right & Top
		lMax = 0;
		lMin = static_cast<long>(static_cast<float>(-m_StageProgInfoHdr.SampleInfo.Size.dy));
		break;
	case 3:		// Left & Bottom
	case 4:		// Right & Bottom
		lMax = static_cast<long>(static_cast<float>(m_StageProgInfoHdr.SampleInfo.Size.dy));
		lMin = 0;
		break;
	}
