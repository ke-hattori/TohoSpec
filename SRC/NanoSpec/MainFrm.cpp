...
/* modified 2015.12.03 hmenjo カメラビデオフォーマット設定化 ---------- } ---------- */
	...
	m_bGetColorInf			= GetOrginalSystemColor();		//起動時のシステムカラーを取り込む
...
// 2009.12.04 K.Matsuo OnCreateから移動 -->
	m_nCameraSelect = 1;							// カメラ選択
	m_nJoyStickMode = 0;							// ジョイスティク選択
// 2009.12.04 K.Matsuo OnCreateから移動 <--
}
...
BOOL CMainFrame::PreTranslateMessage(MSG* pMsg)
{
	...
	case WM_KEYDOWN:
		...
		break;
/* added 2015.03.18 hmenjo FWXGA 対応 ---------- { ---------- */
	case WM_MOUSEWHEEL:
...
// 2013.11.07 Bagus Add (TohoSpec対応) <--
	...
	// 終了時に終了コードを確認するので自動的に削除しないようにする
	...
	/*検索フォルダはResultのみなので１個だけスレッドを立てる*/
...
//	m_pDoc->SetPassageHour(m_pPassageHour.m_span);
	...
	m_MeasMenuEnableFlg = TRUE; //初期化
	m_MeasMenuEnableFlg2 = TRUE; //初期化
	...
	m_nMotionStage	= STAGE_MOTION_HIGH;			// XYの設定値
	m_nMotionElev	= STAGE_MOTION_HIGH;			// Zの設定値

	TestModeTempRcpDelete();
	//Saiki 20100305 Delete ----->
	/*削除スレッドとバッティングして、起動が遅くなる(可能性がある)ため*/
	//OldMeasDataDelete(); //古い測定データファイルを削除
	//Saiki 20100305 Delete <-----
...
//Kojika 20090526 Add End
	...
	///// (2) 親の0行目を 3行 1列 に分割 /////
	// m_wndSplitter(親)の0行目にm_wndSplitter1(子1)を当てはめて、3行1列にする
	// 行列の番号は、0から開始
/* modified 2015.03.18 hmenjo FWXGA 対応 ---------- { ---------- */
...
/* modified 2015.03.18 hmenjo FWXGA 対応 ----------              */
	...
	if ((g_lModelType == MODEL_T3100) && (rcDesktop.bottom < 1024)) {
		/* 共有スクロールバーにするために WS_VSCROLL を追加	*/
...
/* modified 2015.03.18 hmenjo FWXGA 対応 ---------- } ---------- */
	...
	///// (3) 親の1行目を 2行 1列 に分割 /////
	// m_wndSplitter(親)の1行目にm_wndSplitter2(子2)を当てはめて、2行1列にする
	// 行列の番号は、0から開始
#ifndef SPLTR_SHRSCRL	/* modified 2015.04.05 hmenjo FWXGA 対応2 ---------- { ---------- */
	if(!m_wndSplitter2.CreateStatic(&m_wndSplitter, 2, 1, WS_CHILD | WS_VISIBLE | WS_BORDER, m_wndSplitter.IdFromRowCol(0, 1)) ) {
#else					/* modified 2015.04.05 hmenjo FWXGA 対応2 ----------              */
	DWORD l_dwWnd2Styl;
	if ((g_lModelType == MODEL_T3100) && (rcDesktop.bottom < 1024)) {
		/* 共有スクロールバーにするために WS_VSCROLL を追加	*/
...
#endif					/* modified 2015.04.05 hmenjo FWXGA 対応2 ---------- } ---------- */
...
#ifdef SPLTR_SHRSCRL	/* added 2015.04.05 hmenjo FWXGA 対応2 ---------- { ---------- */
		m_wndSplitter2.SetScrollStyle(0);
#endif					/* added 2015.04.05 hmenjo FWXGA 対応2 ---------- } ---------- */
	}
/* added 2015.03.18 hmenjo FWXGA 対応 ---------- } ---------- */
	RECT rect;
	GetClientRect(&rect);
	...
	///// (Pane2) m_wndSplitter1(子1)の1行目,0列目 /////
...
// 2013.11.11 Bagus Add (TohoSpec対応) <--
	...
	///// (Pane5) m_wndSplitter2(子2)の1行目,0列目 /////
...
//Kojika 20090526 Change
	...
	//						"QUIT",
	//if(IDOK == MessageBox(  "NanoSpecを終了してもよろしいですか？",
	//						  "終了",
...
//Kojika 20090526
			dlg.DoModal(); //CMainFrame::OnTimer()内で消す
		}
		else{
			ProcessQuit();
		}
...
//	  }
		...
		// Nanomapが終了するまで待機し、もし5秒待っても終わらない時は強制終了させる。
...
//			MessageBox("NanoPifSock CreateProcess 失敗しました.", "NanoSpec", MB_OK);
...
//			MessageBox("パターン認識の初期化に失敗しました", "NanoSpec", MB_OK);
...
/* added 2013.06.04 hmenjo SubCon PwrOFF ---------- { ---------- */
	if (0 != g_lSBCPwr_OffTime) {
		/* サブコン電源再投入	*/
		nexioSubConPowerOFF(TRUE);		/* サブコン電源 オフ	*/
		::Sleep(g_lSBCPwr_OffTime * 1000);			/* オフ時間	*/
		nexioSubConPowerOFF(FALSE);		/* サブコン電源 オン	*/
		::Sleep(g_lSBCPwr_WaitAfterOn * 1000);		/* オン後起動待ち時間	*/
	}
/* added 2013.06.04 hmenjo SubCon PwrOFF ---------- } ---------- */
// 2009.11.09 K.Matsuo Resist Interlock -->
...
//		MessageBox("Measysの初期化に失敗しました", "NanoSpec", MB_OK | MB_ICONSTOP);
...
//Kojika 20090526
//		::MessageBox(0, _T("Failed to start Chief Thread."), _T("NanoSpec MainFrame"), MB_ICONERROR | MB_SYSTEMMODAL | MB_OK);
//		::MessageBox(0, _T("Chief Threadのスタートに失敗しました."), _T("NanoSpec MainFrame"), MB_ICONERROR | MB_SYSTEMMODAL | MB_OK);
...
//	Event(EV_DO_INIT);
// 2009.11.09 K.Matsuo Resist Interlock -->
// MeaSysの中で、測定ユニット（CTAやロレスタ）のインターロック関係をみている。
// 初期化エラーの場合、危険な状態なので、初期化時のステージ移動は、スキップするようにした。
	if ( bMeasInit == TRUE ) {
	Event(EV_DO_INIT);
	}
// 2009.11.09 K.Matsuo Resist Interlock <--
	...
	//----- Screen	Initialze -----------------------------------------------
	///// Screen Initialize /////
	// ADAPによって変更されたシステムカラーを戻す => OnSysColorChange()
...
/* added 2009.06.02 hmenjo Stage 初期化前の RobotArm ---------- { ---------- */
	// ----- ロボットアーム検出 ---------------------------------------------
	if (0 == nexioIsRobotArmDetect()) {
		// アームが入っています．
/* deleted 2009.06.05 hmenjo NanoSpecTitle 消えない対策 ---------- { ---------- */
//		m_pApp->NanoSpecTitle(NANOSPEC_TITLE_HIDE); 	// タイトルダイアログを消します．
/* deleted 2009.06.05 hmenjo NanoSpecTitle 消えない対策 ---------- } ---------- */
...
//Kojika 20090526 Change
		...
		//if(MessageBox(  "アライメントユニットの初期化に失敗しました\nステージの初期化を実行しますか？",
		//				  "NanoSpec",
...
//Kojika 20090526 Change
				...
				//if(MessageBox(  "アライメントのピンが下端位置にありません\nユニットの初期化を再試行しますか？",
				//				  "NanoSpec",
...
// modified hmenjo 2009.05.12 速度変更許可フラグ追加 ---------- } ----------
#endif	// 初期化時は速度制限はする
		nexioEquipmentStatusRun(TRUE);		// EQ 動作中オン
...
//				MessageBox("ステージ初期化エラー", "NanoSpec", MB_OK | MB_ICONSTOP);
...
// 2013.11.07 Bagus Add (TohoSpec対応) <--
#if 0
		::Sleep(1000);		// 仮で入れてあります．Stage.dll 側で対策出来れば不要です．
#endif
#endif	// ココで速度制限解除する
		nexioEquipmentStatusRun(FALSE); 	// EQ 動作中オフ
	}
...
// 2013.11.07 Bagus Add (TohoSpec対応) <--
		...
		if(bStageInit == TRUE){
			if(bNextraInit == TRUE){
				nexioEquipmentStatusRun(TRUE);		// EQ 動作中オン
				NS_StageInitialize();				// Turret Backlash 除去も、この関数の中で行っている
				nexioEquipmentStatusRun(FALSE); 	// EQ 動作中オフ
			}
		}
	// 2009.07.21 K.Matsuo NSSTAGE <--
...
// 2013.11.07 Bagus Add (TohoSpec対応) <--
	...
	// （ViewWindowViewのイニシャルで使用（bEnable）の一番小さいを設定している。）
// 2013.11.07 Bagus Add (TohoSpec対応) -->
	if(systemConfig.nStageType != STAGE_TYPE_NONE){
// 2013.11.07 Bagus Add (TohoSpec対応) <--
		if(bStageInit == TRUE){
...
/* modified 2012.01.23 hmenjo [８]PatRecEnd()条件 ---------- { ---------- */
//	if(!g_bHWS){
//		PatRecEnd();
//	}
/* modified 2012.01.23 hmenjo [８]PatRecEnd()条件 ----------              */
...
/////////////////////////////////////////////////////////////////////////////
// 測定進行状況（現在の測定ポイント / 測定予定数）
LRESULT CMainFrame::OnSetMeasurementData(WPARAM wparam, LPARAM lparam)
{
	int iPoint = wparam;	// 測定ポイントごとの回数
	CString strBuffer;

	(((CMainFrame*)AfxGetMainWnd())->m_wndSplitter2.GetPane(0, 0))->PostMessage(WM_MEAS_COMP_EVENT, iPoint, 0); // 測定ポイント回数をWParam
...
// Chief Event
// 測定データ格納処理
// (1回呼ばれると1ポイント測定分のデータを格納)
// 測定後のdatファイルはここで作られるものを使用する。
...
// Chief Event
// 1枚測定終了メッセージ受信
// (Seqトランジション→データ処理モジュール)WM_CHIF_MEAS_END
LRESULT CMainFrame::OnOneSheetEnd(WPARAM wparam, LPARAM lparam)
{
	m_pDoc->OperationLogging("1枚測定終了メッセージ受信");
...
// 2009.11.30 K.Matsuo １ポイント測定失敗時のエラー終了通知 -->
...
// 2009.11.30 K.Matsuo １ポイント測定失敗時のエラー終了通知 <--
	...
	if(m_pDoc->GetHostMode() == HOST_REMOTE){
// 2009.09.17 K.Matsuo ストレスの膜厚測定終了時は、データ保存しない -->
		// ストレス測定の膜厚測定は、引き続きライン測定があるので、報告しない
...
// 2009.09.17 K.Matsuo ストレスの膜厚測定終了時は、データ保存しない <--
	...
	HWND l_hChiefDlg = ::FindWindow(0, CHIEF_DLG_CAPTION);
	if (0 != l_hChiefDlg) {
	m_pDoc->OperationLogging("1枚測定終了応答送信");
...
// 2009.09.17 K.Matsuo ストレスの膜厚測定中は、画面遷移させない -->
#if 0
//▽▽▽ 090326 hibino add
#define PROCESS_RETRY_TIME	10000	//(ms)プロセス時のリトライ送信までの待ち時間(若干の誤差有り)
#define ROOP_WAIT			10		//(ms)判定周期 (0は禁止)
	if(m_pDoc->GetHostMode() == HOST_REMOTE){
		for(int i=0; i < (PROCESS_RETRY_TIME/ROOP_WAIT)*2 +1; i++){
			int iProcessStatus = m_pDoc->GetProcessStatus();

			if(iProcessStatus == PROCESS_DOWN){
				m_pDoc->OperationLogging("リモート測定終了後、画面遷移処理時にステータスが『DOWN』");
// 2009.06.04 K.Matsuo -->
//				//ダウン時は測定画面にとどまる。
				// →仕様変更：ダウン時もメイン画面に戻る
				ChangeOperationPanel(OP_MEASUREMENT);
...
// 2009.06.04 K.Matsuo <--
			...
			if((iProcessStatus == PROCESS_COMP)||(iProcessStatus == PROCESS_ABRT)||(iProcessStatus == PROCESS_WAIT)){
				ChangeOperationPanel(OP_MEASUREMENT);
				ChangeExeWnd(RECIPE_MAIN_LIST_WND);
				m_pDoc->OperationLogging("リモート測定終了後、画面遷移処理成功");
				break;
			}
			///// プロセス中 (orイニシャル) /////
			else{
				//画面切替え時にステータスがプロセス中だった場合、一定時間待った後、枚測定終了応答をリトライする
				//それでもプロセス状態が続く場合はアラームを発生させる
				...
				if((i%(PROCESS_RETRY_TIME/ROOP_WAIT) == 0)&&(i != 0)){
					...
					HWND l_hChiefDlg = ::FindWindow(0, CHIEF_DLG_CAPTION);
					if(0 != l_hChiefDlg){
						m_pDoc->OperationLogging("リモート測定終了後、画面遷移処理時『1枚測定終了応答』リトライ実行");
						::PostMessage(l_hChiefDlg, WM_DATA_MEAS_END, 0, 0); //1枚測定終了応答
					}
					else{
						AlarmIf_Set(ALID_OneSampledMeasurementFailed);
...
#endif
	if(m_pDoc->GetHostMode() == HOST_REMOTE){
		// ストレス測定の膜厚測定は、引き続きライン測定があるので、画面遷移させない
		if(rcp_data.MainRcpInfo.MainRcpParam.hdr.wHeadType != HEAD_TYPE_STRESS){
			ChangeOperationPanel(OP_MEASUREMENT);
			ChangeExeWnd(RECIPE_MAIN_LIST_WND);
		}
...
// Chief Event
// 汎用ポジション移動完了メッセージ受信
// (AAFトランジション→画面モジュール)WM_CHIF_MOVE_SETPOS_END
...
// Chief Event
// バキューム指令完了メッセージ受信
// (MSGハンドラ→画面モジュール)WM_CHIF_VACUUM_END
...
// Chief Event
// AFポップアップ指令メッセージ受信
// (AAFトランジション→画面モジュール)WM_CHIF_AF_POPUP
...
LRESULT CMainFrame::OnSrrefEnd(WPARAM wparam, LPARAM lparam)
{
	...
	// Kojika 20090528 Change
	//hWnd = ::FindWindow(NULL, "リファレンス メニュー");
	...
	//リファレンス、ダークリファレンス格納
	if(m_pDoc->GetMeasMode() == MEASMODE_TEST) //テストモード
	{
...
//		m_pDoc->GetTestModeData(&TestModeData, iPoint); //最新ポイント+1のところに書き込むので不要
...
// 		}
		...
		m_pDoc->SetTestModeData(&TestModeData, iPoint + 1); //最新ポイント+1に格納(初回は[0]に格納)
...
// 2010.01.06 bagus Gantry --}--
		...
		if( GetJoyStickMode() != 0 ){
			BOOL bJoyStk= m_pDoc->ChangeJoyStick(TRUE); //ジョイスティックを使用可に設定
			TRACE("CMainFrame::OnSrrefEnd()関数内 ChangeJoyStick()の戻り値:%d\n", bJoyStk);
		}
// ADD 2009.06.01
		// 必ずJoyStickの状態を戻すこと
		m_pDoc->JoyStickStatusRestore();
...
// Chief Event
// AAF終了メッセージ受信
// (MSGハンドラ→画面モジュール)WM_CHIF_AAF_END
...
/////////////////////////////////////////////////////////////////////////////
// MenuExit Exitメニューが呼ばれた後の、戻り画面の表示について定義する
...
// 2010.01.07 bagus Gantry --}--
	...
	// 2009.09.04 bagus stress Stage Stress追加 --{--
...
/////////////////////////////////////////////////////////////////////////////
// CheckSelectMainRecipe レシピ一覧からレシピが選択されているかどうか
...
/////////////////////////////////////////////////////////////////////////////
// ChangeOperationPanel 左側の画面切替え。ビューの削除と作成を行う
void CMainFrame::ChangeOperationPanel(int iMode, int iEditMode /* = EDIT_MAIN_RECIPE */)
{
	// 現在と同じモードなら処理しない
	// ただしMeasurement Mode, Edit Mode, Config Mode, System Mode時にはラジオボタンの初期化を行う
...
//▼▼▼ 090119 hibino 暫定
		m_pDoc->SetDispStatus(MAIN_MENU_MANUAL_MEASUREMENT); //正
//		m_pDoc->SetDispStatus(MAIN_MENU_MEASUREMENT); //仮
//マニュアル測定時は、MAIN_MENU_MANUAL_MEASUREMENTステータスとしますが、
//チーフの動作制限対応が施されるまでは、暫定的にMAIN_MENU_MEASUREMENTステータスとします。
//▲▲▲ 090119 hibino 暫定
		break;
	default:
		pNewViewClass = RUNTIME_CLASS(CMeasurementOperationView);
	...
	// Edit Modeでレシピ編集画面から戻ってきた場合には，直前のリストを表示する
	...
	//メイン画面以外は、常時[ログオン/ログオフ]ボタンを無効にする
	BOOL bButtonEnable;
...
/////////////////////////////////////////////////////////////////////////////
// ChangeExeWnd 右側の画面切替え。ビューの削除と作成を行う
void CMainFrame::ChangeExeWnd(int iExeWnd)
{
	...
	if(m_iCurrExeWnd == iExeWnd){
		SetWindowTitle();
		// MainRecipeList，MultiRecipeListの場合，MeasurementMode，EditModeに応じてツールバーの表示，非表示を行う
...
/* modified 2015.03.18 hmenjo FWXGA 対応 ---------- } ---------- */
	...
	//2009.08.28 bagus stress --{--
	//画面がSR/STRESS共用なのでどちらを意図して呼び出されたかを渡しておく
...
void CMainFrame::ChangeMenu()
{
	...
	MENUSEARCH MenuSearchTable[] =
	{
		...
		// 2013.02.01 bagus CompleteEASEヘッド追加 <--
		// 2009.09.04 bagus stress MainRecipeStress追加 --{--
...
/* deleted 2014.05.22 hmenjo 手動時 NanoMap ---------- { ---------- */
...
/////////////////////////////////////////////////////////////////////////////
// SetWindowTitle 画面に応じたタイトルを編集する
...
/////////////////////////////////////////////////////////////////////////////
// GetSelectListName 各レシピリスト一覧ごとに、選択されていたレシピ名を取得する
...
/////////////////////////////////////////////////////////////////////////////
// SetSelectListName 各レシピリスト一覧ごとに、選択されているレシピ名を保存する
...
/////////////////////////////////////////////////////////////////////////////
// SelectListNameIndex 各レシピ編集画面
// （子）のレシピリスト一覧（親）への紐付け。
// レシピリスト一覧のインデックスを返却する
...
// Purpose	  : SR測定終了の通知
// Parameters : bResult ---> SR測定が正常終了したかどうかの変数
//
// Returns	  : なし
...
/////////////////////////////////////////////////////////////////////////////
// Name 	  : EndSrFocus
// Purpose	  : SRのフォーカス終了の通知
// Parameters : bResult ---> SRのフォーカスが正常終了したかどうかの変数
//
// Returns	  : なし
...
/////////////////////////////////////////////////////////////////////////////
// アプリケーション起動時のシステムカラーを設定
// ADAPによるシステムカラー変更をキャンセルするため）
BOOL CMainFrame::SetOrginalSystemColor()
{
	// OnSysColorChangeで行うので、廃止
	return TRUE;
...
int CMainFrame::ScanDataLabelGet_RS(char szLabel[][ADAPRESULTSTRINGLENMAX + 1], char szLabelOri[][ADAPRESULTSTRINGLENMAX + 1], double dData[ADAPRESULT_COLS_MAX], int iNowPoint, AF_VALID afValid)
{
	...
	//2009.11.17 bagus RS --{--
	//抵抗測定の場合にはOverRange/OverLoadを見る
...
//	AfxMessageBox(_T("AdapResultDataBase Open Error"));
		//AfxMessageBox(_T("AdapResultDataBase オープンエラー"));
...
//	memcpy(szLabel, m_pAdapResultDataBase->GetSharedMemoryPtr()->szLabel, sizeof(m_pAdapResultDataBase->GetSharedMemoryPtr()->szLabel)); //ラベル取得
	memcpy(szLabelOri, m_pAdapResultDataBase->GetSharedMemoryPtr()->szLabelOri, sizeof(m_pAdapResultDataBase->GetSharedMemoryPtr()->szLabelOri)); //DDEのラベル取得

	delete m_pAdapResultDataBase;
	m_pAdapResultDataBase = NULL;

	return iCol-1; //ラベル格納数を返却
}
...
int CMainFrame::ScanDataLabelGet_COMPEASE(char szLabel[][ADAPRESULTSTRINGLENMAX + 1], char szLabelOri[][ADAPRESULTSTRINGLENMAX + 1], double dData[ADAPRESULT_COLS_MAX], int iNowPoint, AF_VALID afValid)
{
	...
	// CompleteEASEの結果に含まれる可能性があるラベルがADAP_DATABASEに設定されているので、
	// 既存のSR,SE処理と互換性をもたせるため、実際に返却されてきたラベル名だけを抽出する
	...
	return iDispLabelCount; //ラベル格納数を返却
}
// 2013.02.01 bagus CompleteEASEヘッド追加 <--
...
/* added 2016.05.05 hmenjo 手動 再測定/指定統計 ---------- { ---------- */
			if (m_iNowPoint <= Point) {
				/* 測定数制限	*/
				/*		本来なら，m_pDoc->GetStoreCount() を使うべきですが，
						m_pDoc->SetStoreCount() が GlassDataStatistics() の
						後に実行されているため，GlassDataStatistics() 内では
						実際の測定数である m_iNowPoint 使わざるを得ません．	*/
				break;
			}
/* added 2016.05.05 hmenjo 手動 再測定/指定統計 ---------- } ---------- */
...
/* added 2016.05.05 hmenjo 手動 再測定/指定統計 ---------- } ---------- */
			...
			if(nFinalFocusFailDataValidOption == 1){ //FanalFocusFailDataValidOption(メインレシピ内)がINVALID
				if(
					// オートフォーカスなし
...
// 2009.09.18 K.Matsuo SE対応＆全体的に見直し -->
/////////////////////////////////////////////////////////////////////////////
// XMP測定結果データを仮フォルダにコピーする。
// コピー先のファイル名には、連番（測定ポイント番号）を付加する。
void CMainFrame::MeasPointDatFileTempCopy(int iNowPoint)
{
	...
	if ( rcp_data.MainRcpInfo.MainRcpParam.hdr.wHeadType == HEAD_TYPE_SR && rcp_data.MeasProgInfo.ScanParams.hdr.wScanType == MEAS_PROG_TYPE_SR_THICKNESS ||
		...
		::CopyFile(pszXmpFittingDataPath, szFittingDataFilePath, FALSE);	// 既存ファイルがあっても、上書きする
	}
}
// 2009.09.18 K.Matsuo SE対応＆全体的に見直し <--
...
BOOL CMainFrame::OnCopyData(CWnd* pWnd, COPYDATASTRUCT* pCopyDataStruct)
{
	...
	TEST_MODE_DATA TestModeData;										//// テストモード用データ
	...
	char szLabelOri[ADAPRESULT_COLS_MAX][ADAPRESULTSTRINGLENMAX + 1];	// DDE受信のタイトル
	...
	int iFormatId;
	int i =0;
	STAGE_COORD_XYZ l_MeasPosXYZ; //ステージ位置取得用(マニュアル測定・テストモードのみ)
...
// 						//但しこのタイミングではまだダークを測定していないので正しいデータは入っていない。
...
// 				}
				...
				///// 測定値(ラベル含む)取得 /////
				//2009.11.04 bagus CA --{--
				//データとラベルの取得処理の共通化（ADAPからのデータとそれ以外（CAなど）
				...
				//2009.11.04 bagus CA --}--
				//2009.11.17 bagus RS --{--
				//統計計算に含めないようにOverRange/OverLoadの場合にはValidを落とす
				if(rcp_data.MainRcpInfo.MainRcpParam.hdr.wHeadType == HEAD_TYPE_4PP
				&& iLabelCount == 0){
				...
				TestModeData.MeasRcpCon.iLens = l_Meas.ScanParams._SR.iLens;						// 測定レンズ
				TestModeData.MeasRcpCon.wOpticsFilterType = l_Meas.ScanParams._SR.wOpticsFilterType;// 光学フィルタ
				TestModeData.MeasRcpCon.dIntegTime = l_Meas.ScanParams._SR.dIntegTime;				// 測定時間
...
// 2009.05.15 k-matsuo -->
//					TestModeData.MeasRcpCon.wStart = MAX_TARGET_WAVELENGTH; 	// wStartは、バグなのでは！？
...
// CHG 2009.06.02 H.Kawamura
			...
			if( GetJoyStickMode() != 0 ){
				BOOL bJoyStk= m_pDoc->ChangeJoyStick(TRUE); //ジョイスティックを使用可に設定
				TRACE("CMainFrame::OnCopyData()関数内 ChangeJoyStick()の戻り値:%d\n", bJoyStk);
			}
#else
			// 必ずJoyStickの状態を戻すこと
			m_pDoc->JoyStickStatusRestore();
...
// CHG 2009.06.02 H.Kawamura
		...
		// ==================================================================
		// 以下、通常測定(リメジャー含む)、マニュアル測定 の処理

		::WaitForSingleObject(m_hEvTabView, INFINITE); //TabViewができるまで待つ
		...
		if(m_pDoc->GetMeasMode() == MEASMODE_SEQ_OR_REMEASURE){ //通常測定orリメジャー
			...
			if(m_pDoc->GetSeqMeasMode() == SEQ_ON_REMEASURE){	//リメジャー中
				//測定キャンセル時に、前の値をそのまま使用する為用に取得
...
/* modified 2016.05.05 hmenjo 手動 再測定/指定統計 ---------- } ---------- */
		...
		OnePointData.afValid.dwValid =((MEAS_1POINT_RESULT*)(pCopyDataStruct->lpData))->dwAFValid;
		AfValidResultJudge(OnePointData.afValid, OnePointData.szShowAfValid, OnePointData.szAfP502);
		// 2009.11.12 K.Matsuo OnePointData.Validは、必ず設定 -->
		OnePointData.Valid = TRUE;
		// 2009.11.12 K.Matsuo OnePointData.Validは、必ず設定 <--
...
/* modified 2016.05.05 hmenjo 手動 再測定/指定統計 ---------- { ---------- */
//			m_pDoc->SetOnePointData(&OnePointData, m_iNowPoint); //OnePointData.dData が統計計算に必要となるので仮登録。
/* modified 2016.05.05 hmenjo 手動 再測定/指定統計 ----------              */
			/* OnePointData.dData が統計計算に必要となるので仮登録	*/
			if (0 < m_pMeaTabViewObj->MRCS_ReMeasureNoGet()) {
				/* 再測定	*/
...
/* added 2016.05.05 hmenjo 手動 再測定/指定統計 ---------- } ---------- */
			...
			///// 測定ポイントデータを仮フォルダにコピーして、適切なファイル名に変更 /////
//			if(m_pDoc->GetMeasMode() == MEASMODE_SEQ_OR_REMEASURE) //通常測定orリメジャー
//			{
/* modified 2016.05.05 hmenjo 手動 再測定/指定統計 ---------- { ---------- */
//			MeasPointDatFileTempCopy(m_iNowPoint);
...
//			}
			...
			if(m_pDoc->GetLabelFlag() == FALSE){
				//最初の1回目の呼び出し時のみ実行するもの
				m_pDoc->SetLabel(szTitleXyz, szTitleAf, szLabel, szLabelOri, iLabelCount); //☆1回で可
...
/* added 2016.05.05 hmenjo 手動 再測定/指定統計 ---------- } ---------- */
/* modified 2016.05.05 hmenjo 手動 再測定/指定統計 ---------- { ---------- */
//		m_pDoc->SetCurrentSetPoint(iSetPointCount); //一番最後にデータを格納(書換え)したポイント番号の格納
//													//リメジャーモード開始時に0をセットすること!
...
/* modified 2016.05.05 hmenjo 手動 再測定/指定統計 ----------              */
		...
		if (0 < m_pMeaTabViewObj->MRCS_ReMeasureNoGet()) {
			...
			m_pMeaTabViewObj->MRCS_ReMeasureNoClr();	/* 再測定番号クリア	*/
			/* 測定番号をセット	*/
...
// 2009.11.25 K.Matsuo GTr Reference <--
		///// サンプルID取得(リモート時のみ) /////
		bGetSamplID = ((SEQ_MEAS_START*)(pCopyDataStruct->lpData))->bGetSamplID;	// FALSE：サンプル ID の指定は無い(画面からのシーケンス測定開始のため)
																			// TRUE ：サンプル ID の指定が有る(Pif からのシーケンス測定開始)
// 2009.11.25 K.Matsuo GTr Reference -->
		if (FALSE == m_bGTRref) {
// 2009.11.25 K.Matsuo GTr Reference <--
			if(bGetSamplID == TRUE){	//リモート時
// 2014.01.17 Bagus Add (Stage None対応) -->
				SYSTEM_CONFIG l_SystemConfig;
				ConfigFile_GetNanoSpecIni(&l_SystemConfig, CONFIG_FILE_SYSTEM_CONFIG);
				if(l_SystemConfig.nStageType != STAGE_TYPE_NONE){
					...
					m_pDoc->SetLastMeasPoint(1); //最終測定ポイントを格納
				}
// 2014.01.17 Bagus Add (Stage None対応) <--
	...
	case WMCD_CHIF_SET_RECIPE: //着工レシピ指示

		///// メモリー初期化 /////
		m_pDoc->InitMeasData();

		strcpy(rcp_data.szRecipeName, (const char*)((COPYDATASTRUCT*)pCopyDataStruct->lpData)); //レシピ名取得
		///// Recipe 取得 /////
...
/////////////////////////////////////////////////////////////////////////////
// 測定結果画面用AF_VALID表示値・上位送信用AF VALID 取得
...
//▲▲▲ 081226 hibino add(TRACE)
	...
	||(afValid.ucResultAF == (BYTE)2)	//AF失敗(自動強制測定)
	||(afValid.ucResultAF == (BYTE)4))	//AF失敗(ポップアップ)
	{
		if(afValid.ucResultPopup == 0)	//ポップアップ無し
		{
			if(afValid.ucResultAF == (BYTE)0)		//AF無し
			{
		...
		else if(afValid.ucResultPopup == 2) //AF OKボタン
		{
			...
			else if((afValid.bit1ManuZ == 0)&&(afValid.bit1ManuAF == 0)) //Zマニュアル操作ナシ かつ AFマニュアル操作ナシ
			{
				...
				else if(afValid.ucResultAF == (BYTE)1)//AF成功
				{
					if(afValid.bit2PopupKind == (BYTE)1)	//ポイント位置補正画面
					{
						strcpy(szShowAfValid, g_lpszAfValidResultShow[1]); //"G(AF)" AF正常
				...
				else if(afValid.ucResultAF == (BYTE)2)//AF失敗(自動強制測定)
				{
					if(afValid.bit2PopupKind == (BYTE)1)	//ポイント位置補正画面
					{
						strcpy(szShowAfValid, g_lpszAfValidResultShow[2]); //"N(AF)" オートフォーカス異常
				...
				else if(afValid.ucResultAF == (BYTE)4)//AF失敗(ポップアップ)
				{
			...
			else if((afValid.bit1ManuZ == 1)&&(afValid.bit1ManuAF == 1)) //Zマニュアル操作アリ かつ AFマニュアル操作アリ
			{
...
//		break;
	case 2L:	// ローカルでなかった
//		MessageBox("ホストモードがローカルではありません。", lpszCaption, MB_OK|MB_ICONWARNING);
		...
		break;;
	case 5L:	// Wait でない
//		MessageBox("アイドル状態ではありません。", lpszCaption, MB_OK|MB_ICONWARNING);
		LoadStringML(IDS_PROCESS_STATUS_NOT_WAIT, strBuffer, "Process Status is not Wait.");
		MessageBox(strBuffer, lpszCaption, MB_OK|MB_ICONWARNING);
		break;;
	case 6L:	// 動作中フラグがオンだった
//		MessageBox("アクチュエータが動作中です", lpszCaption, MB_OK|MB_ICONWARNING);
		LoadStringML(IDS_ACTUATER_MOVING, strBuffer, "Actuater is moving now.");
		MessageBox(strBuffer, lpszCaption, MB_OK|MB_ICONWARNING);
		break;;
	case 7L:	// 画面モードが「測定モード」でなかった
//		MessageBox("画面が測定モードになっていません", lpszCaption, MB_OK|MB_ICONWARNING);
		LoadStringML(IDS_SCREEN_NOT_MEASUREMENT, strBuffer, "Screen is not Measurement Screen.");
		MessageBox(strBuffer, lpszCaption, MB_OK|MB_ICONWARNING);
		break;;
	case 15:	// メンテナンス SW オン && 東朋メンテ SW オフの場合
//		MessageBox("メンテナンススイッチがON かつ 東朋メンテナンススイッチがOFFになってます", lpszCaption, MB_OK|MB_ICONWARNING);
...
// 2009.11.25 K.Matsuo -->
	...
	case 55:
		...
		break;
	case 76:
		LoadStringML(IDS_SAFETY_PLUG_OPEN, strBuffer, "セーフティプラグ開");
		MessageBox(strBuffer,lpszCaption,MB_OK|MB_ICONWARNING);
		break;
// 2009.11.25 K.Matsuo <--
	default:	// その他のエラー
//		MessageBox("測定に失敗しました", lpszCaption, MB_OK|MB_ICONWARNING);
// 2009.11.20 K.Matsuo -->
...
//			MessageBox("メンテナンス日時を経過しました !!!.", "", MB_OK|MB_ICONWARNING);
		}
	}

	// SR Reflectance Lampの使用時間加算
...
//			MessageBox("ランプ交換時期を経過しました !!!.", "", MB_OK|MB_ICONWARNING);
		}
	}
}
...
// 2013.11.07 Bagus Add (TohoSpec対応) <--
	...
	int iSetPointCount; //格納済件数(何ポイント測定したかの数)
	iSetPointCount = m_pDoc->GetStoreCount();


	BOOL bSampleID = rcp_data.MainRcpInfo.MainRcpParam.hdr.bSampleID; //サンプルIDを使用？
	BOOL bSaveMeasResult = rcp_data.MainRcpInfo.MainRcpParam.hdr.bSaveMeasResult; //セーブファイル名にサンプルIDを使用?(bSampleID==TRUE時のみ有効)
	BOOL bSaveOverwrite = rcp_data.MainRcpInfo.MainRcpParam.hdr.bSaveOverwrite; //セーブファイルが同じファイル名だった場合、上書き可？

	BOOL bExitSaveflg = FALSE; //EXITボタン押下時セーブ実行フラグ
	...
	//EXITボタン押下時にセーブしていないものがある場合の処理
	if((m_pMeaTabViewObj->m_bToolbarSaveCompFlg == TRUE) //ツールバーによる測定データ保存有
	&&(!((bSampleID == TRUE)&&(bSaveMeasResult == TRUE)))) //かつ サンプルIDによる自動保存設定でない
	{
		TRACE("%s%s\n", "(CMainFrame::MeasEnd) :", "ツールバーによる測定データ保存有 かつ サンプルIDによる自動保存設定でない"); //←このトレースは削除可 hibino
		//既に保存済みなので、EXIT時には保存しない。
	}
	else if(/*m_pMainFrame->*/m_pMeaTabViewObj->m_bExitSaveCompFlg == FALSE) //測定データ未保存
	{
...
// 2013.11.07 Bagus Mod (TohoSpec対応) -->
//			iRtn = MessageBox(strBuffer, "NanoSpec", MB_YESNOCANCEL); //確認ダイアログ
			LoadStringML(IDS_TITLE_NANOSPEC , strBuffer2, "NanoSpec");
			iRtn = MessageBox(strBuffer, strBuffer2, MB_YESNOCANCEL); //確認ダイアログ
// 2013.11.07 Bagus Mod (TohoSpec対応) <--
			if(iRtn == IDYES)
			{
				bExitSaveflg = TRUE;
			...
			{
				return IDCANCEL; //EXITボタンで抜けず、測定画面のまま
			}

		}

		if(bExitSaveflg == TRUE) //未保存データ保存可
		{
			...
			if((iCheck == 0)||(iCheck == 3)) //保存失敗 OR 保存キャンセル
			{
				//測定画面にとどまる。
				return IDCANCEL;
			}
	...
	//測定モードフラグ設定
	m_pDoc->SetMeasMode(MEASMODE_NOT_MEASURE); //測定中でない(測定結果画面のWAIT時は含まない)
	...
	case MANUAL_MEAS_WND: //←未使用？
		...
		if ( iSetPointCount > 0 ) {
			...
			for ( i = 0; i < ADAPRESULT_COLS_MAX; i++ ) {
				...
				pifIf[i].dUnif = 0.0;								// ゼロでよい。Uniformityは、各社でいろいろな計算ロジックがあるらしい。Procon側で計算する。
			}
		}
...
// 2013.02.01 bagus CompleteEASEヘッド追加 <--
		...
		break;
	default:
		m_pDoc->OperationLogging("CMainFrame::MeasEnd()内 default通過");
...
{
/* modified 2016.05.14 hmenjo 手動 menu バグ ---------- { ---------- */
...
{
//こちらは使いません	m_pMeaTabViewObj->PostMessage(WM_COMMAND, (WPARAM) MAKEWPARAM(IDM_SAVE_AS_RESULT_DATA, 0), (LPARAM) 0);
...
void CMainFrame::OnUpdateNew(CCmdUI* pCmdUI)
{
	...
	if (m_iCurrMode == OP_RECIPE_SETTING) {
		/* レシピ編集モード	*/
		/* ユーザの実行権チェック	*/
/* modified 2016.08.15 hmenjo レシピ編集 menu バグ3 ---------- { ---------- */
...
// =========================================================================
//テストモード用仮レシピ削除(テストモードが異常終了して、残ってしまっていた仮レシピを削除)
...
void CMainFrame::ChangeMaintenanceModeColor(int iMode)
{
	...
	//アクティブウィンドウのタイトルバーの色, アクティブウィンドウの境界の色, メニューの背景色, ウィンドウの枠の色, スクロールバーの軸の色

	int i = 0;

	switch(iMode){
		case MAINFRM_MAINTE_MODE:
			m_bOrgSystemColorChangeExecFlg = FALSE; 	// 色を元に戻されないように設定
			for(i=0; i<5; i++)
			{
...
void CMainFrame::OldMeasDataDelete()
{
	...
	if(bRtnChk == FALSE){ //削除設定なし
		//以降処理なし
		return;
	}

	//カレントディレクトリ操作
	GetCurrentDirectory(sizeof(szDefaultCurrentPath), szDefaultCurrentPath); //検索後に元のカレントディレクトリに戻る為のもの
	SetCurrentDirectory(g_szResult_Dir);//カレントディレクトリを検索対象フォルダに移動

	//削除対象ファイルの検索・削除
	DeleteOldFile(stDeleteBaseTime);

	//カレントディレクトリ操作(元に戻す)
	SetCurrentDirectory(szDefaultCurrentPath); //カレントディレクトリを元(デフォルトのカレント)に戻す
}

/////////////////////////////////////////////////////////////////////////////
// ファイルを削除する基準日を取得
// [戻り値] TRUE:削除対象ファイル検索必要, FALSE:削除の必要なしに設定されている。
//
// [この関数の注意点]
// 曜日情報は使用禁止(曜日情報は正しい値が得られない)。
BOOL CMainFrame::GetDeleteDate(SYSTEMTIME *stDeleteBaseTime)
{
	...
	else if(stSystemTime.wMonth == 2){	//2月
...
/////////////////////////////////////////////////////////////////////////////
// ファイルを削除する基準日以前のものを削除
// [関数の説明]
// 1.『.datファイル』および『←に対応したRAWデータ入りフォルダ』を消す
// 2.上記1.で削除されなかったフォルダ(データ分類用フォルダ)の中へカレントディレクトリを移し、再びこの関数を呼ぶ(再帰)
void CMainFrame::DeleteOldFile(const SYSTEMTIME stDeleteBaseTime)
{
	CString csFileTitle;	//拡張子なしファイル名取得
	CString csFilePath; 	//ファイルパス取得
...
//			csFilePath = cfChkStressData.GetFilePath(); //ファイルパス取得
//			// "." , ".."を無視
...
//				csFilePath = cfFilefind.GetFilePath(); //ファイルパス取得
//				// "." , ".."を無視
...
//				cfFilefind.GetLastWriteTime(ctSearchFileTime);	//ファイルの更新日時を取得
//
//				//削除基準日より古いファイルかどうかのチェック
...
//						/*m_pMainFrame->*/DeleteDirectory(_T(szRawDataDirPath)); //対象ディレクトリを再帰的に削除
//					}
//						//Saiki 20091217 Add ---->
//						/*.datファイルに対応したRAWデータ入りフォルダが存在していなくても、期限の過ぎた.datは消す*/
//						else{
//							DeleteFile(szFilePath);
//						}
//						//Saiki 20091217 Add <----
...
//				csFilePath = cfFilefind2.GetFilePath(); 							// ファイルパス取得
//				// "." , ".."を無視
...
//					GetCurrentDirectory(sizeof(szCurrentPath), szCurrentPath);		// カレントパス取得(検索後に元のカレントディレクトリに戻る為のもの)
//					SetCurrentDirectory(csFilePath);								// カレントディレクトリを検索対象フォルダに移動
//
//					DeleteOldFile(stDeleteBaseTime);								// 再帰
//
//					SetCurrentDirectory(szCurrentPath); 							// カレントディレクトリを元に戻す
//				}
//
//			} while( bFileEnd2 );
//
...
//	if(((bNexMovo2Alarm | bNexMovo2AlarmOld) ^ bNexMovo2Alarm) == TRUE) //立下りを検出
	if(((bNexMovo2Alarm | bNexMovo2AlarmOld) ^ bNexMovo2AlarmOld) == TRUE){ //立ち上がりを検出
		// 装置電源オフ：出力
		AlarmIf_Set(ALID_StageError); //ステージエラー
...
/////////////////////////////////////////////////////////////////////////////
// Chief Event
// 測定ポーズを通知(WM_CHIF_NOTIFY_MEAS_PAUSE)
...
/* added 2009.06.09 hmenjo 異常時測定ポーズ ログ追加 ---------- } ---------- */
	...
	BOOL l_bRelease = (BOOL) lparam;	/* TRUE：ポーズ解除 */

	// ポーズ通知メッセージボックスの存在を確認します．
...
/* modified 2009.06.05 hmenjo ドアポーズアラームはALID 2 ---------- { ---------- */
...
/* deleted 2009.06.08 hmenjo ポーズ前の Resume 不具合対策 ---------- { ---------- */
//			CString l_strPauseMsgBase;
//			LoadStringML(IDS_PAUSEMSGBOX_BASE, l_strPauseMsgBase, _T("Measurement was paused, because %s.\n%s"));
/* deleted 2009.06.08 hmenjo ポーズ前の Resume 不具合対策 ---------- } ---------- */
...
/* added 2009.06.08 hmenjo ポーズ前の Resume 不具合対策 ---------- } ---------- */
						...
						case 31:	/* EMO	*/
						case 33:	/* 装置パワーオフ	*/
//						case 2: 	/* ローカルでなかった	*/未使用になりました(20090603)
						case 3: 	/* Processing でない	*/
						case 4: 	/* 画面モードが「測定モード」か「データモード」でなかった	*/
						case -1:	/* Chief が見つからない */
						default:	/* 未定義エラー */
							OnDoCancel(0, 0);	// キャンセルします．
							break;
						}
					}
					break;
...
// 2009.10.30 K.Matsuo Bug Fix -->
		...
		for ( int nLine = 0; nLine < STRESS_LINES_MAX; nLine++ ) {
			...
			if ( m_pDoc->GetOneStressLineDataValid(nLine + 1) ) {
				...
				pifIf[nLine].dUnif = 0.0;			// ゼロでよい。Uniformityは、各社でいろいろな計算ロジックがあるらしい。Procon側で計算する。
			}
		}
...
/* deleted 2009.08.18 hmenjo ストレス機能追加(46) ---------- { ---------- */
...
BOOL CMainFrame::OutputStressData(LPCTSTR lpszOutputPath,LPCTSTR lpszRecipeName,BOOL bRef)
{
	...
	//データ追記
	m_pMeaTabViewObj->SaveMeasConAdd(lpszRecipeName); //測定データなど追記
...
//	文字列を分割して格納する
// pStrData: 元文字列
// pStrArray: 分割した文字列を格納する
// chSplitter: 分割する文字
// bTrim: 格納する際に両側トリムを行うかどうか
/////////////////////////////////////////////////////////////////////////////
...
BOOL CMainFrame::LoadSERawDataByPointNo(int nPointNo,CSERawDataList *pList)
{
	...
	pList->RemoveAll();
	//計測値
	//測定処理中は測定結果の一時保存ディレクトリに生データがあるのでそのパスを作成する。
	//2009.09.24 bagus se --{--
