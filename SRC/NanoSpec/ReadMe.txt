========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : NanoSpec
========================================================================


AppWizard が生成した NanoSpec には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには NanoSpec を構成している各ファイルの概要説明が含まれています。
...
NanoSpec.h
    ...
    みます） をインクルードし、また CNanoSpecApp アプリケーションクラスを宣
    言します。

NanoSpec.cpp
    このファイルは CNanoSpecApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。
...
res\NanoSpec.ico
    ...
    ルです。このアイコンはリソース ファイル NanoSpec.rc によってインクルード
    されます。
...
res\Toolbar.bmp
    ...
    リソースエディタで編集し、NanoSpec.rc ファイル内の IDR_MAINFRAME TOOLBAR 配列を
    変更してください。
/////////////////////////////////////////////////////////////////////////////

AppWizard は 1 つのドキュメントタイプと 1 つのビューを作成します。

NanoSpecDoc.h, NanoSpecDoc.cpp - ドキュメント
    これらのファイルは CNanoSpecDoc クラスを含みます。特別なドキュメント デ
    ータを付け加えたり、CNanoSpecDoc::Serialize を使ったファイルのセーブや
    ロード機能をインプリメントしたいときはこのファイルを編集してください。

NanoSpecView.h, NanoSpecView.cpp - ビュー
    これらのファイルは CNanoSpecView クラスを含みます。
    CNanoSpecView オブジェクトは CNanoSpecDoc オブジェクトを表示するために
    使います。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) NanoSpec.pch 
