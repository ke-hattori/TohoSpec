...
// 2009.09.17 bagus stress Grid修正 --{--
		if ((DWORD)row >= m_StressConfig.dwLiftPinNumberOfLine + LineFixRows)
			continue;
// 2009.09.17 bagus stress Grid修正 --}--
...
// 2009.09.17 bagus stress Grid修正 --{--
//	for (row=LineFixRows; row<LineRows; row++) {
	for (row=LineFixRows; (DWORD)row<m_StressConfig.dwLiftPinNumberOfLine + LineFixRows; row++) {
// 2009.09.17 bagus stress Grid修正 --}--
...
//		tmpPinX = strtod(Item.strText, NULL) * MICROMETRE;		// 単位変換(um->mm)
		...
		tmpPinX = static_cast<float>(tmpPin * MICROMETRE);
// 2009.09.15 bagus stress 修正 --}--
...
//		tmpPinY = strtod(Item.strText, NULL) * MICROMETRE;		// 単位変換(um->mm)
		...
		tmpPinY = static_cast<float>(tmpPin * MICROMETRE);
// 2009.09.15 bagus stress 修正 --}--
...
// 2009.09.11 bagus stress 修正 --}--
	if (PinOrgY == 0) {
		PinOrgY += m_StressConfig.lInvalidRangeEdge;
		PinY = static_cast<float>(PinOrgY);
	} else if (PinOrgY < 0) {
		PinOrgY += m_StressConfig.lInvalidRangeEdge;
		PinY = (long)((PinOrgY * 1000 - 500) / 1000);
	} else {
...
//		PinX *= -1;
		if (PinOrgX == 0) {
			PinX = static_cast<float>(PinOrgX);
		} else if (PinOrgX < 0) {
			PinX = (long)((PinOrgX * 1000 - 500) / 1000);
		} else {
			PinX = (long)((PinOrgX * 1000 + 500) / 1000);
...
//			m_StressConfig.Line[i].MeasPos[j].lY = PinY;
			if (j== PIN_INTERVAL-1) {
				m_StressConfig.Line[i].MeasPos[j].lX = static_cast<float>(PinMaxX);
...
// Pin Data
		for(j = 0; j<m_StressConfig.Line[i].dwLiftPinNumberOfLn;j++){
			...
			case 1:		// X:→
			default:
				lPinX = static_cast<float>(dOffSetValueX + m_StressConfig.Line[i].PinPos[j].lX);
				break;
			case 2:		// X:↑
				lPinY = static_cast<float>(dOffSetValueX - m_StressConfig.Line[i].PinPos[j].lX);
				break;
			case 3:		// X:←
				lPinX = static_cast<float>(dOffSetValueX - m_StressConfig.Line[i].PinPos[j].lX);
				break;
			...
			case 1:		// Y:→
				lPinX = static_cast<float>(dOffSetValueY + m_StressConfig.Line[i].PinPos[j].lY);
				break;
			case 2:		// Y:↑
			default:
				lPinY = static_cast<float>(dOffSetValueY - m_StressConfig.Line[i].PinPos[j].lY);
				break;
			case 3:		// Y:←
				lPinX = static_cast<float>(dOffSetValueY - m_StressConfig.Line[i].PinPos[j].lY);
				break;
...
// 2009.09.17 bagus stress Grid修正 --}--
		...
		Item.col = 1;
		m_PinListGrid.GetItem(&Item);
		tmpPinX = static_cast<float>(strtod(Item.strText, NULL) * MICROMETRE);		// 単位変換(um->mm)

		Item.mask = GVIF_TEXT;
		Item.row = i;
		Item.col = 2;
		m_PinListGrid.GetItem(&Item);
		tmpPinY = static_cast<float>(strtod(Item.strText, NULL) * MICROMETRE);		// 単位変換(um->mm)
