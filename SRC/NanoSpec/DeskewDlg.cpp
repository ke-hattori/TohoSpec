...
// 2009.06.11 K.Matsuo -->
	...
	case DESKEW_MODE_PATTERN_DESKEW:
		...
		if ( ::GetFileAttributes(szFilePath) == 0xffffffff || ::GetFileAttributes(szFilePath2) == 0xffffffff ) {
			...
			m_pStageProgInfoHdr->nDeskewMode = 0;							// Deskew̎
			m_pStageProgInfoHdr->nDeskewFailOption = 0;						// p^[Fs̐ݒ
...
void CDeskewDlg::OnSelchangeDeskewExecutionMode()
{
	...
	/* DeskewFailOption R{{bNX́ADeskewExecutionMode R{{bNX DESKEW_MODE_PATTERN_DESKEW ̂ƂI\ */
...
{
	// DeskewFailOption R{{bNX́ADeskewExecutionMode R{{bNX DESKEW_MODE_PATTERN_DESKEW ̂ƂI\A
	// lҏWŌĂ΂̂ŁAK[hĂ
	if ( m_pStageProgInfoHdr->nDeskewMode != DESKEW_MODE_PATTERN_DESKEW )
		return;
	...
	// DeskewFailOption  }jADeskewIꂽƂA}jAZbgAbv{^̎gp
