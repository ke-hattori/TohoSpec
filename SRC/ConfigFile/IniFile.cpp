...
// 2009.05.14 k-matsuo -->
// '#' = double		// data type 'D'の精度拡張	double 精度10桁で保存
// 2009.05.14 k-matsuo <--

// SystemCommon.iniのセクション，キー，デフォルト，変数の設定
...
// SystemSR.iniのセクション，キー，デフォルト，変数の設定
...
/* modified 2009.09.29 hmenjo Auto Focus Turret デフォルト ---------- { ---------- */
...
// SystemCompEase.iniのセクション，キー，デフォルト，変数の設定
...
/* SystemStress.iniのセクション，キー，デフォルト，変数の設定 */
DWORD lg_dwDummy;
...
// UserSettingCommon.iniのセクション，キー，デフォルト，変数の設定
...
/* UserSettingSe.ini のセクション，キー，デフォルト，変数の設定	*/
...
// 2013.02.04 bagus CompleteEASEヘッド追加 -->
/* UserSettingEase.ini のセクション，キー，デフォルト，変数の設定	*/
...
/* added 2009.07.22 hmenjo ストレス ライン数 定義 ---------- { ---------- */
				/*	保存の関数を改造する場合は，最大ライン数を超えたラインの定義を ini ファイルに
					書出さないようにしなければならないため，ココと同様な処理にはしないこと．*/
		TCHAR l_tszKey[256];
...
/* added 2009.09.29 hmenjo Auto Focus Turret デフォルト ---------- { ---------- */
	if (SYSTEM_SR_INI_FILE == iIniFile) {
//		for (int iLenss = 1; iLenss <= SR_LENS_MAX; iLenss++) {
		/*	SR_LENS_MAX は「5」だけど，読込みが４(0～3)までしか対応していないので
			「5」にしておきます．	*/
...
/* added 2009.09.29 hmenjo Auto Focus Turret デフォルト ---------- } ---------- */
// 2013.11.14 Bagus Add (TohoSpec対応) -->
...
//		for(nIniStressLine = 0;nIniStressLine < STRESS_LINES_MAX;nIniStressLine++){
		for(nIniStressLine = 0;(DWORD)nIniStressLine < NanoSysData.StressConfig.dwLiftPinNumberOfLine;nIniStressLine++){
// 2009.09.17 bagus Stress --}--
			int nIniStressValidSectionNum = 0;
			int nIniStressValidPinNum = 0;
			char szBuf[1024+1];
...
#else
			for (nIniStressPin=0; (DWORD)nIniStressPin<NanoSysData.StressConfig.Line[nIniStressLine].dwLiftPinNumberOfLn; nIniStressPin++) {
				sprintf(szBuf,"LiftPinLocL%dPin%d_X",nIniStressLine+1,nIniStressPin+1);
				wsprintf(buff,"%ld", NanoSysData.StressConfig.Line[nIniStressLine].PinPos[nIniStressPin].lX);
				WritePrivateProfileString(section,szBuf,buff,szIniFilePath);
...
//			for(nIniStressSection = 0;nIniStressSection < STRESS_SECTIONS_MAX;nIniStressSection++){
			...
			for(nIniStressSection = 0;(DWORD)nIniStressSection < NanoSysData.StressConfig.Line[nIniStressLine].dwLiftPinNumberOfLn - 1;nIniStressSection++){
// 2009.09.17 bagus SE --}--
				sprintf(szBuf,"L%dS%d_ScanStart_X",nIniStressLine+1,nIniStressSection+1);
