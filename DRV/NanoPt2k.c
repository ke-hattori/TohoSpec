...
#define NANO_DEVICE_NAME	L"\\Device\\NanoPt2k"
#define DOS_DEVICE_NAME		L"\\DosDevices\\NanoPt2k"
...
#pragma alloc_text (PAGE, PortAddDevice)
#pragma alloc_text (PAGE, PortDispatchPnp)
#pragma alloc_text (PAGE, PortDispatchSystemControl)
#pragma alloc_text (PAGE, PortUnload)
#pragma alloc_text (PAGE, PortDispatch)
#pragma alloc_text (PAGE, PortIoctlReadPort)
#pragma alloc_text (PAGE, PortIoctlWritePort)
#pragma alloc_text (PAGE, PortStartDevice)
#endif
...
--*/
{
    ...
    DriverObject->MajorFunction[IRP_MJ_CREATE]          = PortDispatch;
    DriverObject->MajorFunction[IRP_MJ_CLOSE]           = PortDispatch;
    DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL]  = PortDispatch;
...
--*/
{
    ...
    RtlInitUnicodeString(&ntDeviceName, NANO_DEVICE_NAME);
...
--*/
{
    ...
    case IRP_MN_START_DEVICE:
        ...
        IoSetCompletionRoutine(Irp,
                               (PIO_COMPLETION_ROUTINE) PortCompletionRoutine, 
                               &event,
                               TRUE,
                               TRUE,
                               TRUE);
        ...
        if (NT_SUCCESS(status) && NT_SUCCESS(Irp->IoStatus.Status)) {
            ...
            status = PortStartDevice(DeviceObject, Irp);
            if(NT_SUCCESS(status))
            {
                //
...

{
    ...
    DebugPrint (("Entered PortDispatch\n"));
    ...
    switch (pIrpStack->MajorFunction)
    {
        ...
        case IRP_MJ_DEVICE_CONTROL:
            ...
            case IO_WRITE_UCHAR:
            case IO_WRITE_USHORT:
            case IO_WRITE_ULONG:
                Status = PortIoctlWritePort(
                            pLDI, 
                            pIrp,
                            pIrpStack,
                            pIrpStack->Parameters.DeviceIoControl.IoControlCode
...

{
	...
	DebugPrint( ("I/O(IN): Address=%XH\n", nPort) );
	nPort &= 0x000000FF;
	IoAddress = *pIOBuffer & 0x0000F000;
	IoAddress |= (ULONG)pLDI->PortBase & 0x00000F00;
	IoAddress |= nPort;
...
*/			*(PUCHAR)pIOBuffer = READ_PORT_UCHAR((PUCHAR)IoAddress);
			DebugPrint( ("I/O Read(BYTE): Add=%XH Data=%XH\n", IoAddress, *pIOBuffer) );
			break;
        case IO_READ_USHORT:
/*            *(PUSHORT)pIOBuffer = READ_PORT_USHORT(
                            (PUSHORT)((ULONG)pLDI->PortBase + nPort) );
*/			*(PUSHORT)pIOBuffer = READ_PORT_USHORT((PUSHORT)IoAddress);
			DebugPrint( ("I/O Read(SHORT): Add=%XH Data=%XH\n", IoAddress, *pIOBuffer) );
            break;
        case IO_READ_ULONG:
/*            *(PULONG)pIOBuffer = READ_PORT_ULONG(
                            (PULONG)((ULONG)pLDI->PortBase + nPort) );
*/			*(PULONG)pIOBuffer = READ_PORT_ULONG((PULONG)IoAddress);
			DebugPrint( ("I/O Read(LONG): Add=%XH Data=%XH\n", IoAddress, *pIOBuffer) );
            break;
        default:      
            return STATUS_INVALID_PARAMETER;
        }
...
/*            *(PUCHAR)pIOBuffer = READ_REGISTER_UCHAR(
                            (PUCHAR)((ULONG)pLDI->PortBase + nPort) );
*/			*(PUCHAR)pIOBuffer = READ_REGISTER_UCHAR((PUCHAR)IoAddress);
            break;
        case IO_READ_USHORT:
/*            *(PUSHORT)pIOBuffer = READ_REGISTER_USHORT(
                            (PUSHORT)((ULONG)pLDI->PortBase + nPort) );
*/			*(PUSHORT)pIOBuffer = READ_REGISTER_USHORT((PUSHORT)IoAddress);
            break;
        case IO_READ_ULONG:
/*            *(PULONG)pIOBuffer = READ_REGISTER_ULONG(
                            (PULONG)((ULONG)pLDI->PortBase + nPort) );
*/			*(PULONG)pIOBuffer = READ_REGISTER_ULONG((PULONG)IoAddress);
            break;
        default:      
            return STATUS_INVALID_PARAMETER;
...

{
	...
	DebugPrint( ("I/O(IN): Address=%XH, Address++=%XH\n", nPort, *pIOBuffer) );

	IoAddress = nPort & 0x0000F000;
	IoAddress |= ((ULONG)pLDI->PortBase & 0x00000F00);
	nPort &= 0x000000FF;
	IoAddress |= nPort;
...
/*            WRITE_PORT_UCHAR(
                (PUCHAR)((ULONG)pLDI->PortBase + nPort),
                *(PUCHAR)pIOBuffer );
*/			WRITE_PORT_UCHAR((PUCHAR)IoAddress, *(PUCHAR)pIOBuffer);
			DebugPrint( ("I/O Write(BYTE): Add=%XH Data=%XH\n", IoAddress, *pIOBuffer) );
            break;
        case IO_WRITE_USHORT:
/*            WRITE_PORT_USHORT(
                    (PUSHORT)((ULONG)pLDI->PortBase + nPort),
                    *(PUSHORT)pIOBuffer );
*/          WRITE_PORT_USHORT((PUSHORT)IoAddress, *(PUSHORT)pIOBuffer);
			DebugPrint( ("I/O Write(SHORT): Add=%XH Data=%XH\n", IoAddress, *pIOBuffer) );
            break;
        case IO_WRITE_ULONG:
/*            WRITE_PORT_ULONG(
                (PULONG)((ULONG)pLDI->PortBase + nPort),
                *(PULONG)pIOBuffer );
*/			WRITE_PORT_ULONG((PULONG)IoAddress, *(PULONG)pIOBuffer);
			DebugPrint( ("I/O Write(LONG): Add=%XH Data=%XH\n", IoAddress, *pIOBuffer) );
            break;
        default:      
            return STATUS_INVALID_PARAMETER;
        }
...
/*            WRITE_REGISTER_UCHAR( 
                    (PUCHAR)((ULONG)pLDI->PortBase + nPort),
                    *(PUCHAR)pIOBuffer );
*/			WRITE_REGISTER_UCHAR((PUCHAR)IoAddress, *(PUCHAR)pIOBuffer);
            break;
        case IO_WRITE_USHORT:
/*            WRITE_REGISTER_USHORT(
                    (PUSHORT)((ULONG)pLDI->PortBase + nPort),
                    *(PUSHORT)pIOBuffer );
*/            WRITE_REGISTER_USHORT((PUSHORT)IoAddress, *(PUSHORT)pIOBuffer );  
			break;
        case IO_WRITE_ULONG:
/*            WRITE_REGISTER_ULONG(
                    (PULONG)((ULONG)pLDI->PortBase + nPort),
                    *(PULONG)pIOBuffer );
*/			WRITE_REGISTER_ULONG((PULONG)IoAddress, *(PULONG)pIOBuffer );
            break;
        default:      
            return STATUS_INVALID_PARAMETER;
        }
