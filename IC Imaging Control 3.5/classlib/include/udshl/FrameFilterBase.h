...
namespace _DSHOWLIB_NAMESPACE
{
	...
	UDSHL_EXP_API_	smart_com<IFrameFilter>		createChain_( const dvector<IFrameFilter*>& list );
	...
	inline HRESULT	IFrameFilterBase::_getSupportedInputTypes( FrameTypeInfo** ppTypeArr, long* pCount ) const
	{
        ...
        *pCount = 0;
		...
		FrameTypeInfo* type_arr = (FrameTypeInfo*) CoTaskMemAlloc( sizeof( FrameTypeInfo ) * arr.size() );
		if( type_arr == 0 )			return E_OUTOFMEMORY;
