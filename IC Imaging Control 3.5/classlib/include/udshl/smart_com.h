...
public:
    ...
    smart_com( smart_com&& op2 ) : m_pComPtr( op2.m_pComPtr )
    {
        op2.m_pComPtr = NULL;
    }
...
#endif
	smart_com&		operator=( const smart_com& op2 )
	{
        destroy();
        m_pComPtr = op2.m_pComPtr;
        if( m_pComPtr ) {
            m_pComPtr->AddRef();
        }
		return *this;
    ...
    bool            empty() const { return m_pComPtr == NULL; }
    ...
    pointer*		get_pp() { return &m_pComPtr; }
    void**          get_ppv() { return (void**) &m_pComPtr; }

	/**	return an addref'ed interface pointer. Needed when you want to pass an AddRef'ed pointer by value. */
	pointer			getReference() const { if( !empty() ) { get()->AddRef(); } return get(); }
	...
	void			destroy()	{ if ( !empty() ) { m_pComPtr->Release(); m_pComPtr = NULL; } }

    void            swap( smart_com& op2 ) { std::swap( m_pComPtr, op2.m_pComPtr ); }
    void            reset()     { destroy(); }
public:
	template<class TType>
...
template<class TComObj>
inline bool	operator==( const void* p, const smart_com<TComObj>& op2 )
{
	return op2.get() == p;
}
