...
namespace _DSHOWLIB_NAMESPACE
{
...
#if _MSC_VER >= 1400
			wcsncpy_s( *ppDataOut, ws.length() + 1, ws.c_str(), cbSettings / sizeof( ws[0] ) );
#else
...
#if _MSC_VER >= 1400
			wcsncpy_s( desc.name, name.c_str(), (sizeof( desc.name ) / sizeof( desc.name[0] )) - 1 );
#else
	...
	private:
		...
		struct param_info : public param_info_base
		{
			...
			{
				if( !param_info_base::isCompatible( value ) )
					return eINCOMPATIBLE_PARAM_VALUE;
		...
		{
			typename tParamInfoMap::iterator it = m_paramInfoLookup.find( name );
		...
		{
            typename tDataInfoMap::iterator it = m_dataInfo.find( name );
		...
		virtual	bool	enumParameters_( unsigned int n, IParameterCtrl::ParamDesc* pPD ) const
		{
			...
			if( n >= m_paramInfoList.size() )
			{
                ...
                typename tDataInfoMap::const_iterator it = m_dataInfo.begin();
				std::advance( it, m );
	...
	protected:
		virtual	void	getFilterInfo( FilterInfo* pInfo ) const
		{
			...
			*pInfo = T::getStaticFilterInfo();
			...
			{
#if _MSC_VER >= 1400
				wcsncpy_s( pInfo->module, m_moduleName.c_str(), sizeof( pInfo->module ) / sizeof( pInfo->module[0] ) );
#else
