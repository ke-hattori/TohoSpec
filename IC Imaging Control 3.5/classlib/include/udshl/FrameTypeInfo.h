...
namespace _DSHOWLIB_NAMESPACE
{
    ...
    {
		FrameTypeInfo();
        FrameTypeInfo( tColorformatEnum fmt );
        FrameTypeInfo( const GUID& subtype );
        FrameTypeInfo( tColorformatEnum fmt, unsigned int width, int height, size_t buf_size = 0 );
		FrameTypeInfo( tColorformatEnum fmt, const SIZE& dim, size_t buf_size = 0 );
		FrameTypeInfo( const GUID& subtype, unsigned int width, int height, size_t buf_size = 0 );
		FrameTypeInfo( const GUID& subtype, const SIZE& dim, size_t buf_size = 0 );
	...
	public:
        ...
        explicit FrameTypeInfoArray( const dvector<FrameTypeInfo>& vec ) { assign( vec.begin(), vec.end() ); }


        template<class TIter>
        FrameTypeInfoArray( TIter assign_beg, TIter assign_end ) : std::vector<FrameTypeInfo>( assign_beg, assign_end ) {}
	 ...
	 **/
	inline size_t	        getBitsPerPixel( tColorformatEnum colorformat );

	UDSHL_EXP_API_ size_t	getBitsPerPixel( const GUID& subtype );
	...
	size_t	                calcBufferSize( const SIZE& size, tColorformatEnum colorformat );
    size_t	                calcBufferSize( const SIZE& size, const GUID& subtype );
	...
	UDSHL_EXP_API_ size_t						calcRequiredBuffersize( const FrameTypeInfo& type );
	/** Creates a BITMAPINFOHEADER for the specified frame type, if possible. */
	UDSHL_EXP_API_ smart_ptr<BITMAPINFOHEADER>	createBitmapInfoHeader( const FrameTypeInfo& type );

	UDSHL_EXP_API_ dstringw	    convertToString_( const FrameTypeInfo& type );

	/** Creates a string representation of the FrameTypeInfo object passed. */
	inline std::wstring		    convertToStringW( const FrameTypeInfo& type )   { return convertToString_( type ); }
	inline std::string		    convertToStringA( const FrameTypeInfo& type )   { return wstoas( convertToString_( type ) ); }
	inline std::string			convertToString( const FrameTypeInfo& type )    { return convertToStringA( type ); }

	UDSHL_EXP_API_ dstringw		convertSubtypeToString_( const GUID& type );
	 ...
	 */
	inline std::wstring		        convertSubtypeToStringW( const GUID& type )     { return convertSubtypeToString_( type ); }
	 ...
	 */
	UDSHL_EXP_API_ GUID				convertStringToSubtype( const dstringw& type );
	 ...
	 */
	UDSHL_EXP_API_ FrameTypeInfo	convertToFrameTypeInfo( const dstringw& type );
	 ...
	 */
    UDSHL_EXP_API_ GUID		createFCCMediaSubType( char ch0, char ch1, char ch2, char ch3 );
    inline GUID				createFCCMediaSubType( const wchar_t* pFCC ) { return createFCCMediaSubType( (char)pFCC[0], (char)pFCC[1], (char)pFCC[2], (char)pFCC[3] ); }
    inline GUID				createFCCMediaSubType( const char* pFCC ) { return createFCCMediaSubType( pFCC[0], pFCC[1], pFCC[2], pFCC[3] ); }
    ...
    inline FrameTypeInfo::FrameTypeInfo( tColorformatEnum fmt ) : subtype( getSubtypeFromColorformat( fmt ) ), dim( 0, 0 ), buffersize( 0 ) {}
    inline FrameTypeInfo::FrameTypeInfo( const GUID& fmt ) : subtype( fmt ), dim( 0, 0 ), buffersize( 0 ) {}
    inline FrameTypeInfo::FrameTypeInfo( tColorformatEnum fmt, unsigned int width, int height, size_t buf_size )
		: subtype( getSubtypeFromColorformat( fmt ) ), dim( width, height ), buffersize( buf_size )	{}
	inline FrameTypeInfo::FrameTypeInfo( tColorformatEnum fmt, const SIZE& dim, size_t buf_size )
		: subtype( getSubtypeFromColorformat( fmt ) ), dim( dim ), buffersize( buf_size )	{}
	inline FrameTypeInfo::FrameTypeInfo( const GUID& subtype, unsigned int width, int height, size_t buf_size )
		: subtype( subtype ), dim( width, height ), buffersize( buf_size )	{}
	inline FrameTypeInfo::FrameTypeInfo( const GUID& subtype, const SIZE& dim, size_t buf_size )
		: subtype( subtype ), dim( dim ), buffersize( buf_size )	{}

	inline tColorformatEnum	FrameTypeInfo::getColorformat()	const       { return getColorformatFromSubtype( subtype ); }
	inline size_t		    FrameTypeInfo::getBitsPerPixel()	const   { return _DSHOWLIB_NAMESPACE::getBitsPerPixel( subtype ); }
	inline bool				FrameTypeInfo::isBottomUpFrameType() const  { return _DSHOWLIB_NAMESPACE::isBottomUpFrameType( subtype ); }
    ...
    inline size_t           calcBytesPerLine( const GUID& subtype, int width )                  { return (getBitsPerPixel( subtype ) * (size_t)abs( width )) / 8; }
    inline size_t           calcBytesPerLine( tColorformatEnum colorformat, int width )         { return (getBitsPerPixel( colorformat ) * (size_t)abs( width )) / 8; }
    inline size_t           calcBytesPerLine( const FrameTypeInfo& info )                       { return calcBytesPerLine( info.subtype, info.dim.cx ); }

    inline size_t	        calcBufferSize( const SIZE& size, tColorformatEnum colorformat )    { return calcBytesPerLine( colorformat, size.cx ) * abs( size.cy ); }
    inline size_t	        calcBufferSize( const SIZE& size, const GUID& subtype )             { return calcBytesPerLine( subtype, size.cx ) * abs( size.cy ); }

}

#endif // FRAMETYPEINFO_H_INC_
